...
async fn insert_and_list_are_scoped_per_user(pool: PgPool) {
    ...
    repo.insert_key(&user(USER_A), &key)
        .await
    ...
    assert_eq!(a_keys[0].expose(), key.expose());
...
async fn count_keys_counts_only_caller_rows(pool: PgPool) {
    ...
    repo.insert_key(&user(USER_A), &UserApiKey::from_raw("key-a-1"))
        .await
        .expect("insert A");
    repo.insert_key(&user(USER_A), &UserApiKey::from_raw("key-a-2"))
        .await
        .expect("insert A again");
    repo.insert_key(&user(USER_B), &UserApiKey::from_raw("key-b-1"))
        .await
        .expect("insert B");
