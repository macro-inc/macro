...
#[sqlx::test(
    ...
    fixtures(
        ...
        scripts("message_annotations")
    )
)]
async fn document_reader_returns_shared_threads_with_imported_authors(
    pool: Pool<Postgres>,
) -> anyhow::Result<()> {
    let repo = PgDocumentRepo::new(pool);
    let threads = repo.get_document_comments("document-with-comments").await?;
    assert_eq!(threads.len(), 4);
    let first = threads
        .iter()
        .find(|t| t.root.id == uuid::Uuid::from_u128(1))
        .unwrap();
    assert_eq!(first.root.content, "Initial question on page 1");
    assert_eq!(
        first.root.imported_author.as_ref().unwrap().name,
        "Original PDF author"
    );
    ...
    assert!(
        threads
            .iter()
            .find(|t| t.root.id == uuid::Uuid::from_u128(2))
            .unwrap()
            .state
            .resolved
    );
    assert!(
        repo.get_document_comments("missing-document")
            .await?
            .is_empty()
    );
    Ok(())
...
async fn document_reader_preserves_root_tombstones_and_excludes_deleted_threads(
    pool: Pool<Postgres>,
) -> anyhow::Result<()> {
    ...
    let repo = PgDocumentRepo::new(pool);
    let parent = MessageParent::parse("document", "document-with-comments")?;
    messages.delete(&parent, uuid::Uuid::from_u128(1)).await?;
    let threads = repo.get_document_comments("document-with-comments").await?;
    let first = threads
        .iter()
        .find(|t| t.root.id == uuid::Uuid::from_u128(1))
        .unwrap();
    assert!(first.root.deleted_at.is_some());
    assert!(first.root.content.is_empty());
    assert_eq!(first.replies.len(), 1);
    assert!(first.state.anchor.is_some());
    messages
        .delete_thread(&parent, uuid::Uuid::from_u128(1))
        .await?;
    let threads = repo.get_document_comments("document-with-comments").await?;
    ...
    assert!(
        threads
            .iter()
            .all(|t| t.root.id != uuid::Uuid::from_u128(1))
    );
    Ok(())
}
