...
async fn attach_document_to_email_attachment(
    pool: &Pool<Postgres>,
    document_id: &str,
    attachment_id: Uuid,
) -> Result<()> {
    ...
    sqlx::query!(
        r#"INSERT INTO "User" (id, email, name, macro_user_id)
           VALUES ($1, 'projection-owner@example.com', 'Projection Owner', $2)"#,
        user_id.to_string(),
        user_id
    )
    .execute(pool)
    ...
    sqlx::query!(
        r#"INSERT INTO "Document" (id, name, owner, "fileType", uploaded, "createdAt", "updatedAt")
           VALUES ($1, 'linked.pdf', $2, 'pdf', true, NOW(), NOW())"#,
        document_id,
        user_id.to_string()
    )
    .execute(pool)
...
async fn provider_reconciliation_preserves_attachment_linked_to_a_document(
    pool: Pool<Postgres>,
) -> Result<()> {
    let message_id = Uuid::parse_str("00000000-0000-0000-0000-00000000b504")?;
    let document_id = "00000000-0000-0000-0000-00000000dc01";
    let mut attachments = vec![service_attachment(
        "linked.pdf",
        "application/pdf",
        "provider-linked",
    )];
    let attachment_id = attachments[0].db_id;
    let mut tx = pool.begin().await?;
    crate::attachments::provider::insert_attachments(&mut tx, message_id, &mut attachments).await?;
    tx.commit().await?;
    attach_document_to_email_attachment(&pool, document_id, attachment_id).await?;

    // The provider no longer reports the attachment. Reconciliation retains
    // the source row because deleting it would cascade away document_email.
    let mut tx = pool.begin().await?;
    crate::attachments::provider::insert_attachments(&mut tx, message_id, &mut []).await?;
    tx.commit().await?;

    assert!(
        sqlx::query_scalar!(
            "SELECT EXISTS(SELECT 1 FROM email_attachments WHERE id = $1)",
            attachment_id
        )
        .fetch_one(&pool)
        .await?
