...
// update_thread_metadata computes is_signal as part of its thread-row update.
#[sqlx::test(
    migrator = "MACRO_DB_MIGRATIONS",
    fixtures(path = "../../../fixtures", scripts("sync_thread_signal_flag"))
)]
async fn update_thread_metadata_clears_signal_flag(pool: Pool<Postgres>) -> anyhow::Result<()> {
...
async fn update_thread_metadata_sets_signal_flag(pool: Pool<Postgres>) -> anyhow::Result<()> {
    let thread_id = Uuid::parse_str(SIG_THREAD_UNLABELED)?;
    let link_id = Uuid::parse_str("00000000-0000-0000-0000-000000000d01")?;
...
async fn update_thread_metadata_skips_unchanged_row(pool: Pool<Postgres>) -> anyhow::Result<()> {
    let thread_id = Uuid::parse_str(SIG_THREAD_UNLABELED)?;
    let link_id = Uuid::parse_str("00000000-0000-0000-0000-000000000d01")?;
    let mut conn = pool.acquire().await?;
    ...
    assert_eq!(
        thread_after.updated_at, thread_before.updated_at,
        "unchanged metadata should not rewrite the thread"
    );
    Ok(())
}
