...
1. **Macro workspace** — `ListEntities` sorted by `recently_updated` with a `df` updatedAt window for the day: documents edited, projects touched, calls joined. Query finished work with `ListTasks` (`status: ["completed"]`, `assignee: "me"`, `updatedAfter`/`updatedBefore` for the day's bounds).
