    ...
    Logs: AgentSessionLogWriter + AgentSessionRepo,
{
    ...
    pub(crate) async fn next_input(&mut self) -> Input<SessionCompletion> {
        loop {
            ...
            // failed or stalled flush is a failed log write, and the machine
            // decides what that means, same as a failed append.
            () = log_flush_due => match tokio::time::timeout(
            ...
            .await
            {
                ...
                Err(_) => {
                    tracing::error!(
                        id = %self.machine.id(),
                        "agent session timed out flushing buffered log frames"
                    );
                    Input::Closed(CloseReason::LogFailed)
                }
            },
