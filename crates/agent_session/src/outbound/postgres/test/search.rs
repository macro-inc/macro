...
async fn search_metadata_is_scoped_current_and_omits_deleted_sessions(pool: PgPool) {
    ...
    repo.set_name(session.id, "Updated searchable name")
        .await
        .unwrap();
    let rows = crate::outbound::postgres::search::search_metadata(&pool, &[session.id.as_uuid()])
        .await
        .unwrap();
    ...
    assert!(
        crate::outbound::postgres::search::search_metadata(&pool, &[])
            .await
            .unwrap()
            .is_empty()
    );
    AgentSessionRepo::delete(&repo, session.id).await.unwrap();
    let rows = crate::outbound::postgres::search::search_metadata(
        &pool,
        &[session.id.as_uuid(), other.id.as_uuid()],
    )
    .await
    .unwrap();
    assert_eq!(rows.len(), 1);
