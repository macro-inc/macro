...
where
    Engine: TurnEngine,
{
    with_agent_connector(engine, Arc::new(crate::domain::mcp::NoMcpServers), scenario).await
}

async fn with_agent_connector<Engine: TurnEngine, Out>(
    engine: Arc<Engine>,
    connector: Arc<dyn crate::domain::mcp::McpToolConnector>,
    scenario: impl AsyncFnOnce(ConnectionTo<Agent>, SessionId) -> Out,
) -> (Vec<SessionNotification>, Out) {
...
impl crate::domain::mcp::McpToolConnector for Arc<SpyConnector> {
    async fn connect(
        ...
        _input: Option<SharedUserInputRequester>,
    ) -> Result<Option<mcp_toolset::RemoteMcpToolSet>, String> {
        self.asked
            .lock()
            .expect("asked lock")
...
/// Session creation dials every advertised HTTP server, including Macro.
#[tokio::test]
async fn session_new_dials_macro_and_other_advertised_servers() {
...
struct FormEngine;
impl TurnEngine for FormEngine {
    fn run_turn(
        &self,
        request: TurnRequest,
    ) -> tokio::sync::mpsc::Receiver<Result<StreamPart, agent::AgentError>> {
        let (parts, receiver) = tokio::sync::mpsc::channel(1);
        tokio::spawn(async move {
            let input = request.user_input.unwrap();
            let forms = (0..2).map(|index| {
                let input = input.clone();
                async move {
                    input.form(format!("macro: Review {index}"), serde_json::json!({
                        "type":"object", "properties":{"draft":{"type":"string"}}
                    }), Some(serde_json::json!({"macro":{"userTool":{"name":"SendEmail","draft":{"subject":"hello"}}}}).as_object().unwrap().clone())).await.unwrap()
                }
            });
            let responses = tokio::select! {
                _ = request.cancel.cancelled() => return,
                responses = futures::future::join_all(forms) => responses,
            };
            let _ = parts
                .send(Ok(StreamPart::Content(
...
async fn concurrent_mcp_forms_queue_and_wait_beyond_the_turn_idle_deadline() {
    let (notifications, asked, response) = with_asking_engine(
        ...
        async |connection, session| {
            connection
                .send_request(text_prompt(&session, "review drafts"))
                .block_task()
                .await
                .unwrap()
        },
    ...
    assert_eq!(asked[0].message, "macro: Review 0");
    assert_eq!(asked[1].message, "macro: Review 1");
    assert_eq!(
        serde_json::to_value(&asked[0]).unwrap()["_meta"]["macro"]["userTool"]["name"],
        "SendEmail"
    );
    assert_eq!(response.stop_reason, StopReason::EndTurn);
    assert_eq!(
        spoken(&notifications),
        r#"[{"action":"decline"},{"action":"decline"}]"#
    );
}

struct FailingReconnect(std::sync::atomic::AtomicUsize);
#[async_trait::async_trait]
impl crate::domain::mcp::McpToolConnector for FailingReconnect {
    async fn connect(
        &self,
        _: Vec<agent_client_protocol::schema::v1::McpServerHttp>,
        _: Option<SharedUserInputRequester>,
    ) -> Result<Option<RemoteMcpToolSet>, String> {
        if self.0.fetch_add(1, Ordering::SeqCst) == 0 {
            Ok(None)
        } else {
            Err("MCP unavailable".into())
        }
    }
}

#[tokio::test]
async fn failed_session_setup_preserves_the_previous_binding_and_history() {
    let engine = Arc::new(ScriptedEngine::new(vec![StreamPart::Content(
        "remembered".into(),
    )]));
    with_agent_connector(
        engine.clone(),
        Arc::new(FailingReconnect(Default::default())),
        async |connection, session| {
            connection
            ...
            connection
                .send_request(text_prompt(&session, "second"))
                .block_task()
                .await
                .unwrap();
        },
    ...
    let requests = engine.requests();
    assert_eq!(requests.len(), 2);
    assert_eq!(requests[1].messages, vec!["first", "remembered", "second"]);
}
