...
#[sqlx::test(migrator = "MACRO_DB_MIGRATIONS")]
async fn creating_a_webhook_bot_ignores_has_agent_and_is_not_an_agent(
    pool: PgPool,
) -> anyhow::Result<()> {
    ...
    let bot = read_bot(create_response).await;
    ...
    let patch_request = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            serde_json::json!({ "has_agent": true, "name": "Renamed Bot" }).to_string(),
        ))
        .unwrap();

    let patch_response = router.oneshot(patch_request).await.unwrap();
    assert_eq!(patch_response.status(), StatusCode::OK);
    let patched = read_bot(patch_response).await;
    assert_eq!(patched.name, "Renamed Bot");
    assert!(!patched.has_agent);
