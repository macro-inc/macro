...
#[derive(Debug)]
pub struct DatabaseAccessLevelExtractor<T: RequiredPermission, Svc, Auth> {
    /// The entity access receipt for the database.
    pub entity_access_receipt: EntityAccessReceipt<T>,
    _marker: PhantomData<(T, Svc, Auth)>,
}

impl<T, S, Svc, Auth> FromRequestParts<S> for DatabaseAccessLevelExtractor<T, Svc, Auth>
where
    ...
    S: Send + Sync + 'static,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let Path(DatabaseAccessParams { id: database_id }) = parts
            .extract::<Path<DatabaseAccessParams>>()
            .await
