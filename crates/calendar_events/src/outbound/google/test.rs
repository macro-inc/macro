...
#[test]
fn full_day_out_of_office_timed_span_reads_back_as_all_day() {
    let event = |event_type: Option<&str>, start: &str, end: &str| -> GoogleEvent {
        let mut value = serde_json::json!({
            "id": "provider-event",
            "iCalUID": "ooo@example.com",
            "summary": "Away",
    ...
    // Google stores an all-day out-of-office event as a midnight-to-midnight
    // timed span; it must read back as all-day, in the offset's local dates.
    assert_eq!(
        google_time(&event(
            Some("outOfOffice"),
        ...
        .unwrap(),
        EventTime::AllDay {
            start_date: NaiveDate::from_ymd_opt(2026, 9, 17).unwrap(),
            end_date: NaiveDate::from_ymd_opt(2026, 9, 18).unwrap(),
        }
    );

    // A genuinely timed out-of-office event keeps its times.
    assert!(matches!(
        google_time(&event(
            Some("outOfOffice"),
    ...
    // A non-out-of-office midnight-to-midnight span is left timed.
    assert!(matches!(
        google_time(&event(
            None,
            "2026-09-17T00:00:00-04:00",
