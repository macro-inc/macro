...
pub struct CorpusTask {
    ...
    #[serde(default)]
    pub body: String,
    /// Task owner, **anonymized** for prod snapshots: the real `macro|…@macro.com`
    /// id is replaced with a fictitious `macro|name@macro.com` before the corpus
    /// is written, so no real employee email is committed. `None` for synthetic
    /// tasks.
...
///
/// These mirror the cases called out in the dedup spec. The expected-duplicate
/// cases stress different *kinds* of positive so recall isn't measured on
/// paraphrases alone: `Rephrasing` (same words, reordered), `TerseVsDetailed`
/// (a title-only stub vs a full writeup of the same work), and
/// `LowLexicalOverlap` (same work with almost no shared vocabulary — the case
/// pure lexical matching misses and embeddings must carry). The expected
/// NON-duplicate cases (`SameProjectDifferentAction`,
/// `SameActionDifferentIntegration`) guard against over-merging, plus catch-alls
...
pub enum PairCase {
    ...
    Rephrasing,
    /// A terse, title-only task vs a detailed writeup of the same work — expected
    /// duplicate. Stresses length asymmetry, which skews embedding similarity.
    TerseVsDetailed,
    /// Two descriptions of the same work sharing almost no vocabulary (synonyms,
    /// different framing) — expected duplicate. The case a lexical pre-filter
    /// would miss, so it isolates whether the embedding captures meaning.
    LowLexicalOverlap,
...
impl PairCase {
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            PairCase::TerseVsDetailed => "terse_vs_detailed",
            PairCase::LowLexicalOverlap => "low_lexical_overlap",
