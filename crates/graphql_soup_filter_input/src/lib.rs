...
enum GraphqlAgentSessionLiteral {
    /// Opt this query into agent sessions at all. Sessions are off by default,
    /// so without this (or an `id`) Soup omits them entirely. Must be `true`;
    /// there is no literal for excluding sessions, that is the default.
    Include(bool),
    /// The id option.
    Id(ID),
