...
async fn same_team_refresh_drops_inherited_rows_outside_the_tree(
    pool: PgPool,
) -> Result<(), sqlx::Error> {
    let mut tx = pool.begin().await?;
    acquire_guard(&mut tx).await?;
    let project_id = Uuid::from_u128(0x20000000_0000_0000_0000_000000000001);
    let team_id = Uuid::from_u128(0x10000000_0000_0000_0000_000000000001);
    ...
    assert!(
        inherited
            .iter()
            .all(|row| row.entity_id != stale_id && row.access_level == AccessLevel::View)
    );
    assert_eq!(
        inherited
            .iter()
            .map(|row| (row.entity_id, row.entity_type.as_str()))
            .collect::<Vec<_>>(),
        vec![
            (
                Uuid::from_u128(0x20000000_0000_0000_0000_000000000004),
                "chat"
            ),
            (
                Uuid::from_u128(0x20000000_0000_0000_0000_000000000002),
                "document"
            ),
            (
                Uuid::from_u128(0x20000000_0000_0000_0000_000000000005),
                "email_thread"
            ),
            (
                Uuid::from_u128(0x20000000_0000_0000_0000_000000000003),
                "project"
            ),
        ]
    );
