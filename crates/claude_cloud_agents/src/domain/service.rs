        ...
        loop {
            ...
            match event {
                Some(Ok(event)) => {
                    ...
                    // The demo never grants permissions automatically. Respond through the
                    // actual control protocol so a worker cannot hang waiting for approval.
                    if payload["type"] == "control_request"
                        && payload["request"]["subtype"] == "can_use_tool"
                    {
                        self.cloud.send(&self.id, json!({"type":"control_response", "response":{
                            "subtype":"success", "request_id":payload["request_id"],
                            "response":{"behavior":"deny","message":"Tool approval is not enabled in the Macro Claude demo"}
                        }})).await?;
                    }
