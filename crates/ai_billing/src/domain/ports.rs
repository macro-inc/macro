...
/// Reads quota-consuming AI usage at Macro's list rate, not all measured cost.
pub trait UsageReader: Send + Sync + 'static {
    /// List-rate usage for each of `users` within `period`.
    ///
    /// Exclude all [`super::QUOTA_EXEMPT_FEATURES`]: AI projections, document
    /// editing, and dictation remain recorded for cost telemetry but consume
    /// none of a user's allowance, credits, or overage.
