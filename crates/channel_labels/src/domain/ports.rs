...
pub trait ChannelLabelsRepo: Send + Sync + 'static {
    ...
    /// is restricted to non-DM channels `viewer` participates in, regardless of
    /// channel type or owning team. Manual counts include all non-DM assignments;
    /// smart counts include only the viewer's active matches.
    ...
    /// [`super::models::can_label_channel`] must allow its channel kind;
    ...
    /// [`super::models::can_label_channel`]; removals may clear historical
    ...
    /// regardless of the channel's owning team.
