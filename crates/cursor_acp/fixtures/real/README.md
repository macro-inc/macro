...
```bash
cargo build -p cursor_acp --bin cursor_acp
CURSOR_API_KEY=... CURSOR_ACP_RECORD_DIR=recordings/<label> target/debug/cursor_acp
```
...
Then `cargo test -p cursor_acp && cargo insta review` to accept the new
fixture's snapshots.
