//! The `cursor_acp` binary: an ACP agent backed by Cursor cloud agents.
//!
...
//!   committing one (`crates/cursor_acp/fixtures/real/README.md`).
//! - `RUST_LOG`: tracing filter (default `info`; `debug` logs every frame
//!   and SSE event).

use cursor_acp::cursor::{ApiKey, CursorClient, CursorConfig};
use cursor_acp::domain::model::RepoUrl;
use cursor_acp::domain::service::CursorSessionService;
use cursor_acp::inbound::acp::{AcpNotifier, AcpWriter, serve};
use cursor_acp::outbound::git::GitRepoResolver;
...
async fn main() -> ExitCode {
    ...
    let Ok(api_key) = CursorApiKey::new() else {
        eprintln!("cursor_acp: CURSOR_API_KEY is not set");
    ...
    S: tracing::Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
{
    ...
    if let Err(error) = std::fs::create_dir_all(&dir) {
        eprintln!(
            "cursor_acp: cannot create log dir {}: {error}",
            dir.display()
        );
        return None;
    }
    ...
    match std::fs::File::create(&path) {
        Ok(file) => {
            ...
            eprintln!("cursor_acp: logging to {}", path.display());
            Some(
                tracing_subscriber::fmt::layer()
        ...
        Err(error) => {
            eprintln!("cursor_acp: cannot open {}: {error}", path.display());
            None
        }
    }
}
