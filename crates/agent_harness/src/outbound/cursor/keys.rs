...
/// Resolves a user's Cursor API key.
pub trait CursorApiKeys: Send + Sync + 'static {
    /// The key `owner` registered.
    ///
    /// # Errors
    ...
    ) -> impl Future<Output = Result<CursorApiKey>> + Send;
}

/// Reads keys from `cursor_api_keys`, decrypting through `cipher`.
#[derive(Clone)]
...
{
    async fn resolve(&self, owner: &MacroUserIdStr<'_>) -> Result<CursorApiKey> {
        let owner = owner.as_ref();
        let stored = cursor_api_key::store::get_cursor_api_key(&self.pool, owner)
            .await
            .map_err(|error| {
                HarnessError::Container(format!("could not read a cursor api key: {error}"))
            })?
