...
fn message_capability() -> EntityAccessReceipt<MessageWrite> {
...
#[tokio::test]
async fn bot_posts_with_a_triggering_user_keep_acting_user_attribution() {
    let mut messages = messages::domain::api::MockMessageServiceApi::new();
    messages.expect_post().once().returning(|_, input| {
    ...
    service
        .post_message(
            ...
            PostMessageRequest {
                ...
                triggered_by: Some("macro|sender@test.com".into()),
            },
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn typing_translates_action_and_thread() {
    let mut messages = messages::domain::api::MockMessageServiceApi::new();
    messages
        .expect_typing()
        .once()
        .returning(|_, root, active, nonce| {
            assert_eq!(root, Some(Uuid::from_u128(502)));
            assert!(!active);
            assert_eq!(nonce.as_deref(), Some("t1"));
            Ok(())
        });
    let service = ChannelMessageAdapter::new(Arc::new(messages));
    service
        .post_typing(
            ...
            PostTypingRequest {
                ...
                thread_id: Some(Uuid::from_u128(502).to_string()),
                nonce: Some("t1".into()),
            },
        )
        .await
