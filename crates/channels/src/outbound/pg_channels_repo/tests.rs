use crate::domain::models::{
    AttachmentEntityReference, ChannelType, ChannelWithParticipants, CreateChannelRequest,
    CreateEntityMentionOptions, GetChannelsParams, GetChannelsRequest, GetThreadReplyRowsRequest,
...
async fn channel_thread_rows_are_visible_to_channel_members(
    ...
    let parent_ids = rows.iter().map(|row| row.message.id).collect::<Vec<_>>();
    assert_eq!(parent_ids, vec![MSG3, MSG1, MSG31]);

    let msg1 = rows
        .iter()
        .find(|row| row.message.id == MSG1)
...
async fn channel_thread_rows_filter_by_thread_id(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    assert_eq!(rows[0].message.id, MSG1);
...
async fn threads_matching_participant(
    ...
    let rows = repo(pool)
        ...
        .await
        .map_err(report_err)?;
    Ok(rows.iter().map(|row| row.message.id).collect())
}
...
async fn channel_thread_rows_apply_cursor(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    assert_eq!(first_page[0].message.id, MSG3);
            ...
            GetThreadReplyRowsRequest {
                ...
                query: Query::Cursor(Cursor {
                    ...
                    val: CursorVal {
                        ...
                        last_val: first_page[0].message.updated_at,
                    },
                    filter: None,
                }),
            }
