...
/// A user stop settles the open agent message and emits a control, so one
/// frame reports two changes. A consumer that only applied the last event
/// would keep rendering the thought as in-flight.
#[test]
fn a_stop_reports_the_settled_turn_and_the_control() {
    ...
    assert_eq!(
        consumer.messages[1].stop,
        Some(StopReason::Cancelled),
        "the thought is settled in the same push as the Stopped line"
    );
}
