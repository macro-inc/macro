...
pub async fn fetch_pdf_placeable_anchors(
    ...
    sqlx::query_as!(
        ...
        JOIN comms_message_threads t ON pa.root_id = t.root_id
...
pub async fn fetch_pdf_highlight_anchors(
    ...
    sqlx::query_as!(
        ...
        LEFT JOIN comms_message_threads t ON ph.root_id = t.root_id
        WHERE ph."documentId" = $1
        AND ph."deletedAt" IS NULL
        AND t.deleted_at IS NULL
        GROUP BY ph.uuid, ph.owner, ph.root_id, ph.page, ph.red, ph.green, ph.blue, ph.alpha, ph.type, ph.text, ph."pageViewportWidth", ph."pageViewportHeight", ph."createdAt", ph."updatedAt", ph."deletedAt"
        "#,
        document_id
    )
    .fetch_all(db)
