...
async fn setup(pool: &PgPool) {
    ...
    for (kind, id, level) in [
        (EntityType::Document, DOCUMENT, AccessLevel::Comment),
    ...
    sqlx::query!(
        r#"UPDATE "Document" SET "projectId" = $1, "deletedAt" = NOW() WHERE id = $2"#,
        PROJECT,
        DOCUMENT
    )
    .execute(tx.as_mut())
    ...
    sqlx::query!(
        ...
        VALUES ($1, 'document', 'macro|other@example.com', 'user', 'view')"#,
        Uuid::parse_str(DOCUMENT).unwrap()
    )
    .execute(tx.as_mut())
    .await
...
async fn legacy_restore_preserves_consent_but_never_recreates_cleaned_share(pool: PgPool) {
    ...
    let entity = EntityType::Document.with_entity_string(DOCUMENT.to_string());
    // Exercise both an accurate caller hint and one that omitted the persisted parent.
    for (cleaned, hint) in [(false, Some(PROJECT)), (true, None)] {
        let mut tx = pool.begin().await.unwrap();
        let facts = load_facts(&mut tx, &entity).await.unwrap();
        if cleaned {
            // T15 lifecycle cleanup uses canonical maintenance, not current membership.
        ...
        tx.commit().await.unwrap();
        revert_delete_document(&pool, DOCUMENT, hint).await.unwrap();
        let mut tx = pool.begin().await.unwrap();
        assert_eq!(load_facts(&mut tx, &entity).await.unwrap(), before);
        assert_eq!(before.current.is_none(), cleaned);
        let row = sqlx::query!(
            r#"SELECT "projectId" as project_id, "deletedAt" as deleted_at FROM "Document" WHERE id = $1"#,
            DOCUMENT
        )
        .fetch_one(tx.as_mut())
        .await
        ...
        let grants = sqlx::query!("SELECT source_type::text AS source_type, granted_from_project_id FROM entity_access WHERE entity_id = $1", Uuid::parse_str(DOCUMENT).unwrap())
...
async fn legacy_restore_uses_current_parent_not_stale_hint(pool: PgPool) {
    setup(&pool).await;
    // A stale deleted parent supplied by a caller must not detach an active parent.
    sqlx::query!(r#"INSERT INTO "Project" (id, name, "userId", "deletedAt") VALUES ('stale-parent', 'Stale', 'macro|owner@example.com', NOW())"#)
        .execute(&pool).await.unwrap();
    revert_delete_document(&pool, DOCUMENT, Some("stale-parent"))
        .await
        .unwrap();
    assert_eq!(
        sqlx::query_scalar!(
            r#"SELECT "projectId" as project_id FROM "Document" WHERE id = $1"#,
            DOCUMENT
        )
        .fetch_one(&pool)
        .await
    ...
    assert_eq!(sqlx::query_scalar!("SELECT count(*) FROM entity_access WHERE entity_id = $1 AND granted_from_project_id = $2", Uuid::parse_str(DOCUMENT).unwrap(), PROJECT)
...
async fn legacy_restore_rolls_back_when_inheritance_cleanup_fails(pool: PgPool) {
    ...
    sqlx::raw_sql("CREATE FUNCTION reject_cleanup() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF OLD.granted_from_project_id IS NOT NULL THEN RAISE EXCEPTION 'injected cleanup failure'; END IF; RETURN OLD; END $$; CREATE TRIGGER reject_cleanup BEFORE DELETE ON entity_access FOR EACH ROW EXECUTE FUNCTION reject_cleanup();")
        .execute(&pool).await.unwrap();
    assert!(
        revert_delete_document(&pool, DOCUMENT, Some(PROJECT))
            .await
            .is_err()
    );
    let row = sqlx::query!(
...
async fn permanent_deletion_removes_canonical_bookkeeping_and_all_grants(pool: PgPool) {
    setup(&pool).await;
    crate::document::delete_document::delete_document(&pool, DOCUMENT)
        .await
        .unwrap();
    assert_eq!(
        sqlx::query_scalar!(r#"SELECT count(*) FROM "SharePermission" WHERE id = 'document'"#)
            .fetch_one(&pool)
            .await
            .unwrap(),
        Some(0)
    ...
    assert_eq!(
        sqlx::query_scalar!(
            r#"SELECT count(*) FROM "DocumentPermission" WHERE "documentId" = $1"#,
            DOCUMENT
        )
        .fetch_one(&pool)
        .await
