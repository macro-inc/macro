...
-- current state of the linked pull request, not a history of captures. The patch
...
CREATE TABLE agent_session_changes (
    ...
    -- in application code).
    changeset_id UUID,
    source TEXT CHECK (source IN ('github_pull_request')),
    ...
    attempt_outcome TEXT CHECK (attempt_outcome IN ('captured', 'not_ready', 'failed')),
    attempt_error TEXT,
