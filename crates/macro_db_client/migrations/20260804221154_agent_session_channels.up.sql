...
ALTER TABLE comms_channels
    ...
    ADD CONSTRAINT comms_channels_kind_check
        CHECK (kind IN ('normal', 'agent'));

CREATE TABLE agent_session (
    id                UUID        PRIMARY KEY,
    channel_id        UUID        NOT NULL UNIQUE
                                  REFERENCES comms_channels(id) ON DELETE CASCADE,
    thread_id         UUID
                                  REFERENCES comms_messages(id) ON DELETE SET NULL,
