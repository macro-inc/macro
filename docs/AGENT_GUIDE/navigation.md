...
Settings → Agents and Settings → Runtimes render while their requests are pending.
...
Settings → Runtimes, above Cursor, with the
Anthropic logo. Settings → Agents selects an agent's runtime but does not host
