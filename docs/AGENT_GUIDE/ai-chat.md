...
ordinary forms. External MCP clients see a multiline email preview with editable addresses, subject,
and an optional replacement body; the Macro web UI keeps its email composer. Concurrent questions appear one at a time. If a server session
