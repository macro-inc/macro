...
The Agents list includes owned and shared sessions. Rows show persona, session
title, and status; opening one navigates to `/app/agent/<id>`. The session title
...
sessions, searchable by title or persona). These inline chips show persona,
session name, and last-known status, and open the existing session when clicked.
...
grant. Inaccessible sessions render a private/deleted label. Owners receive live status events; other viewers see the
periodic preview refresh while the browser tab is active.
