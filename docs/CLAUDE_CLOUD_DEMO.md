...
2. Select **Claude Cloud**. It is offered for new selections only when model discovery confirms that your Macro identity has a configured connection. When editing a saved Claude agent, the harness stays visibly selected while discovery loads or fails. Models belong to the selected harness: Fable is available through Claude Cloud only, and switching to Macro Agent replaces the catalog and model preference with that harness's choices.
