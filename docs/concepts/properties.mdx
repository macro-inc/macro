...
Properties are Macro's unified system for attaching structured fields to entities. The same machinery powers a document's metadata, a [task's](/concepts/blocks#tasks) status and assignee, an email thread's custom fields, and a [CRM](/concepts/blocks#crm--contacts--companies) contact's phone number — so the type system, editors, and behaviors below apply everywhere properties appear.
...
- **Tasks** — Status, Priority, Assignees, Due Date, Parent Task / Subtasks, Depends On, Effort, Story Points, and Relevant Documents. See [Blocks → Tasks](/concepts/blocks#tasks).
- **CRM contacts & companies** — Email, Name, Company, interaction timestamps, Domains, Email Sync, and more. See [Blocks → CRM](/concepts/blocks#crm--contacts--companies).
