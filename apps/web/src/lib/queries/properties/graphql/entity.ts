...
import { makeGraphqlEntitySoupInput } from '../../soup/graphql/entity-input';
import {
...
/**
 * Builds an exact-entity Soup query through the shared query-filter compiler.
...
export function buildEntityPropertiesInput(
  ...
  const soupType = match(entityType)
    .with('DOCUMENT', 'TASK', () => 'document' as const)
    .with('PROJECT', () => 'project' as const)
    .with('CHAT', () => 'chat' as const)
    .with('THREAD', () => 'emailThread' as const)
    .with('CHANNEL', () => 'channel' as const)
    .with('CALL_RECORD', () => 'call' as const)
    .with('COMPANY', () => 'crmCompany' as const)
    .with('CALENDAR_EVENT', () => 'calendarEvent' as const)
    .with('USER', () => undefined)
    .exhaustive();

  return soupType
    ? makeGraphqlEntitySoupInput([{ id: entityId, type: soupType }])
    : undefined;
}
