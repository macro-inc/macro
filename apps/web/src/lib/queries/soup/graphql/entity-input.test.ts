...
describe('batched entity Soup inputs', () => {
  it('targets exact IDs across types and excludes unrelated branches', () => {
    ...
    expect(input).toMatchObject({ initial: {
      limit: 4, expand: true, emailView: 'ALL',
      filters: {
        documentFilter: { or: { left: { literal: { id: 'doc-1' } }, right: { literal: { id: 'doc-2' } } } },
  ...
  it('has deterministic variables and balanced trees for a full batch', () => {
    const entities = Array.from({ length: 50 }, (_, i) => ({ entityType: 'DOCUMENT' as const, entityId: `doc-${i}` }));
  ...
  it('does not issue empty or unsupported lookups', () => {
    ...
    expect(buildGraphqlEntitiesSoupInput([{ entityType: 'USER', entityId: 'user' }])).toBeUndefined();
  });
});
