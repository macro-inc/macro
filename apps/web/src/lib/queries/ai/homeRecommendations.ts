      ...
      .array(
        z.object({
          entityType: z
            .enum(entityTypes)
            .describe(
              'Exact notification entityType, or email_thread for QuerySoup email results'
            ),
          entityId: z
            .string()
            .min(1)
            .max(512)
            .describe(
              'Exact notification entityId, or exact email thread id returned by QuerySoup and inspected with GetThread'
            ),
          title: z
            .string()
            .trim()
...
const RECOMMENDATION_PROMPT = [
  ...
  `Gather email candidates by calling QuerySoup exactly once with query { soup(input: { emailPreset: SIGNAL, emailView: "inbox", limit: ${TRIAGE_INPUT_LIMIT} }) { items { id ... on GraphqlSoupEmailThread { inboxVisible isRead name snippet senderEmail } } } }. This is the canonical email source: inboxVisible determines whether an email is active and isRead is its read state. Do not look up or validate emails through ListNotifications.`,
  "If QuerySoup returns email candidates, call ListInboxes exactly once. The current user's email addresses are the returned inboxes where isDelegated is false; use them only to recognize whether a message was addressed directly to the current user.",
  'Before recommending any email, call GetThread with its exact QuerySoup id as threadId and limit 1. GetThread returns messages most recent first: read messages[0].bodyParsed in full and inspect messages[0].from, messages[0].to, and messages[0].cc. The QuerySoup snippet and GetThread summary are not substitutes for reading bodyParsed. Never recommend an email if GetThread fails, returns no latest message, or bodyParsed is missing or insufficient to establish an action.',
  ...
  'For a non-email item, copy entityType and entityId exactly from ListNotifications. For an email, set entityType to "email_thread" and entityId to the exact email id from QuerySoup that you inspected with GetThread. Also set an action, a terse reason tied to the latest message, and a concrete prompt instruction grounded in that message\'s actual sender, ask, and context — never a generic template.',
