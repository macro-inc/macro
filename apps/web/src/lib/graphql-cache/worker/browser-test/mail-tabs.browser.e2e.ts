...
test('Drafts, Sent, Calendar and Shared use canonical metadata offline', async ({
  ...
  try {
    await page.getByLabel('View', { exact: true }).selectOption('DRAFTS');
    ...
    await page.getByLabel('View', { exact: true }).selectOption('SENT');
    await expect(rows).toHaveCount(10);
    await expect(rows.first()).toContainText('Sent 68');
    await more.click();
    await expect(rows).toHaveCount(16);
    await page.getByLabel('View', { exact: true }).selectOption('ALL');
    await page.getByLabel('Calendar', { exact: true }).selectOption('true');
    await expect(rows).toHaveCount(10);
    await more.click();
    await expect(rows).toHaveCount(14);
    await page.getByLabel('Calendar', { exact: true }).selectOption('all');
    await page.getByLabel('Sharing', { exact: true }).selectOption('ONLY');
    ...
    await page.getByLabel('Account', { exact: true }).selectOption('1000');
