...
describe('CacheWorkerCore', () => {
  ...
  it('records bounded Soup projection and filter rollout telemetry', async () => {
    const observations: Array<Record<string, unknown>> = [];
    const entityFilter = vi.fn().mockResolvedValue({
      kind: 'unsupported',
      reason: 'literal',
    });
    ...
    loadCacheWasmMock.mockResolvedValue({
      openCache: vi.fn().mockResolvedValue({ entityFilter, hydrateQuery }),
    });
    const core = new CacheWorkerCore({
      telemetry: {
    ...
    await core.handleRequest(port, {
      ...
      data: { user: { id: 'redacted-by-design' } },
    });

    expect(observations).toContainEqual(
      expect.objectContaining({
        name: 'graphql_cache.soup_filter',
        soupFilterOutcome: 'unsupported',
        soupUnsupportedReason: 'literal',
      })
    );
    expect(observations).toContainEqual({
