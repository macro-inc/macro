...
Create row relationships with AddColumn.linkToTableId, then INSERT/DELETE (row_id,linked_id) edges through the exact relation.junctionSqlName when relation.writable. A relation column's writable=false describes its read-only JSON projection; it does not forbid permitted junction edits. Never UPDATE the projected relation cell. Removing a link must not delete the target row. Resolve target row IDs by reading the target table and use baseVersions for both endpoint tables when editing relationships.
