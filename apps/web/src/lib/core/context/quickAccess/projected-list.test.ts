...
function root<T>(fn: () => T): T {
  return createRoot((dispose) => { cleanups.push(dispose); return fn(); });
}
function document(id: string, bucket = 'note'): SearchDocumentWire {
  return { profile: 'quick-access-v1', recordKey: `GraphqlSoupDocument:${id}`, bucket, searchText: id, timestampMs: 1, sourceHash: 'hash' };
}
function page(ids: string[], more = false): SearchCachePage {
  return {
    documents: ids.map((id) => document(id)),
    nextCursor: more ? { recordKey: `GraphqlSoupDocument:${ids.at(-1)}`, timestampMs: 1 } : null,
  };
}
const materialize = async (documents: SearchDocumentWire[]) => documents.map((document) => ({ id: document.recordKey }));
const ids = (count: number, start = 0) => Array.from({ length: count }, (_, i) => `${start + i}`);
...
describe('Quick Access local projection', () => {
  it('follows the local browse cursor, deduplicates hits, and stops at the last page', async () => {
    const search = vi.fn<(args: SearchCacheArgs) => Promise<SearchCachePage>>()
      .mockResolvedValueOnce(page(ids(50), true))
      .mockResolvedValueOnce(page(['49', ...ids(30, 50)]));
    const list = root(() => createProjectedList({ host: { search }, buckets: ['note'], revision: () => 0, materialize }));
    await vi.waitFor(() => expect(list.items()).toHaveLength(50));
    expect(list.hasMore()).toBe(true);
    await list.loadMore();
    expect(search.mock.calls[1][0]).toMatchObject({ query: '', limit: 50, cursor: { recordKey: 'GraphqlSoupDocument:49', timestampMs: 1 } });
  ...
  it('replays the loaded window after hydration, retaining visible rows while it refreshes', async () => {
    ...
    const list = root(() => createProjectedList({ host: { search }, buckets: ['note'], revision, materialize }));
  ...
  it('searches only materializable buckets before applying the limit', async () => {
    const emailHits = Array.from({ length: 500 }, (_, i) => document(`email-${i}`, 'email'));
    const search = vi.fn(async (args: SearchCacheArgs): Promise<SearchCachePage> => ({
      documents: [...emailHits, document('target')].filter((hit) => args.buckets?.includes(hit.bucket)).slice(0, args.limit), nextCursor: null,
    }));
    const list = root(() => createProjectedList({ host: { search }, buckets: exclude('person'), revision: () => 0, searchTerm: () => 'target', materialize }));
    await vi.waitFor(() => expect(list.items()).toEqual([{ id: 'GraphqlSoupDocument:target' }]));
    expect(search.mock.calls[0][0].buckets).not.toEqual(expect.arrayContaining(['email']));
    expect(search.mock.calls[0][0].buckets).not.toEqual(expect.arrayContaining(['crm_company']));
    expect(search.mock.calls[0][0].buckets).not.toEqual(expect.arrayContaining(['agent_session']));
  });

  it('never interprets an unsupported-only list as an all-bucket search', async () => {
    const search = vi.fn();
    const list = root(() => createProjectedList({ host: { search }, buckets: ['email', 'person', 'crm_company', 'agent_session'], revision: () => 0, materialize }));
  ...
  it('continues past incomplete or duplicate pages instead of stranding scroll pagination', async () => {
    const search = vi.fn<(args: SearchCacheArgs) => Promise<SearchCachePage>>()
      ...
      .mockResolvedValueOnce(page(['one'], true))
      .mockResolvedValueOnce(page(['two']));
    const list = root(() => createProjectedList({ host: { search }, buckets: ['note'], revision: () => 0,
      materialize: async (documents) => materialize(documents.filter((doc) => !doc.recordKey.endsWith('incomplete'))),
    }));
  ...
  it('drops an obsolete in-flight page when the query changes', async () => {
    ...
    const list = root(() => createProjectedList({ host: { search }, buckets: ['note'], revision: () => 0, searchTerm: query, materialize }));
    await vi.waitFor(() => expect(list.items()).toHaveLength(1));
    const loading = list.loadMore();
    setQuery('new');
    expect(list.items()).toEqual([]);
    await vi.waitFor(() => expect(list.items()).toEqual([{ id: 'GraphqlSoupDocument:new' }]));
  ...
  it('disabling and disposing prevent late results from populating a list', async () => {
    ...
    const list = root(() => createProjectedList({ host: { search }, buckets: ['note'], revision: () => 0, enabled, materialize }));
    setEnabled(false);
