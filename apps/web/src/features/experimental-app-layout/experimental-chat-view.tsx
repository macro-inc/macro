...
const CHAT_HISTORY_QUERY: SoupItemsQueryArgs = {
  params: { sort_method: 'updated_at', limit: 100 },
  body: { ...QUERY_FILTERS_BASE, chat_filters: undefined },
};

function isChatEntity(entity: EntityData): entity is ChatEntity {
  return entity.type === 'chat';
}

function ExperimentalChatHistoryItem(props: {
  chat: ChatEntity;
  active: boolean;
  onOpen: () => void;
}) {
  ...
  return (
    ...
    >
      <span class="flex size-4 shrink-0 items-center justify-center">
        <Show when={unread()}>
          <PulsingStar kind="listIcon" opened />
        </Show>
      </span>
      <span class="min-w-0 flex-1 truncate text-sm font-medium">
        {props.chat.name || 'Untitled chat'}
      </span>
      <span class="shrink-0 text-xs font-light text-ink-extra-muted opacity-0 transition-opacity group-hover/chat:opacity-100">
...
function ChatWorkspaceMain(props: {
