...
export function PersonaList(props: {
  ...
  return (
    <SettingsPage
      title="Agents"
      description="Named agents you can mention. Each one runs with its own instructions."
      actions={
    ...
    >
      <SettingsSection
        title="Your agents"
        description="Mentioning an agent opens a session it answers in. Your agents are private to you."
      >
        <SettingsCard>
          <Show
            when={!props.loading}
              ...
              fallback={
                <div class="flex min-h-52 flex-col items-center justify-center px-8 text-center">
                  ...
                  <div class="mt-1 max-w-80 text-xs text-ink-muted">
                    Give it a name, a handle and instructions, and put it to
                    work by mentioning it in any channel.
                  </div>
                  <Button
                    class="mt-4"
                    variant="cta"
