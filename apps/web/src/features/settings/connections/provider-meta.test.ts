...
it('hides Featured-owned slugs from Pipedream Browse', () => {
  for (const slug of ['github', 'linear', 'notion', 'slack', 'cursor']) {
  ...
  expect(PIPEDREAM_BROWSE_HIDDEN_SLUGS.has('gmail')).toBe(false);
});

it('keeps harnesses out of Featured and Browse', () => {
  expect(EMPTY_STARTERS.map((item) => item.id)).toEqual([
    'github',
    'linear',
    'notion',
    'slack',
  ]);
  expect(isPipedreamBrowseHidden('cursor')).toBe(true);
});
