...
function setup(initial: Partial<AgentDraft> = {}, editing = false) {
  return createRoot((dispose) => {
    ...
    const [runtimes, setRuntimes] = createSignal<AgentRuntime[]>([
      {
        id: 'in-memory',
        name: 'Macro',
        kind: 'builtin',
    ...
    return { state, save, setCatalog, setPending, setRuntimes };
  });
}

describe('createAgentDraft', () => {
  it('retains saved bypass only while the runtime still permits it', async () => {
    const { state, save, setRuntimes } = setup(
      { runtimeId: 'computer', autoAcceptPermissions: true },
      true
    );
    const computer: AgentRuntime = {
      id: 'computer',
      name: 'My computer',
      kind: 'macrod',
      description: 'Your computer',
      allowPermissionBypass: true,
      target: { harness: 'macrod', harnessId: 'computer' },
    };
    setRuntimes([computer]);

    expect(state.dirty()).toBe(false);
    expect(await state.save()).toBe(true);
    expect(save).toHaveBeenLastCalledWith(
      expect.objectContaining({ autoAcceptPermissions: true })
    );

    setRuntimes([{ ...computer, allowPermissionBypass: false }]);
    expect(state.autoAcceptPermissions()).toBe(false);
    expect(await state.save()).toBe(true);
    expect(save).toHaveBeenLastCalledWith(
      expect.objectContaining({ autoAcceptPermissions: false })
    );
  });
