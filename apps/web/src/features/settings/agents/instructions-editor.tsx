...
export function AgentInstructionsEditor(props: {
  ...
  const config = buildConfig('markdown')
    ...
    .onChange((markdown) => {
      ...
      // Drafts store standard Markdown; the shell compares its richer internal
      // format. Track both so accepting an edit does not re-import the content.
      appliedMarkdown = editorStateAsMarkdown(
        config.controls.getLexical(),
        'external'
      );
