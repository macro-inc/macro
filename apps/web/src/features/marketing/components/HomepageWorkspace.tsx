import { AgentRosterRow } from '@app/features/agents-view/components/AgentRosterRow';
import { Message as AgentMessage } from '@app/features/block-agent/component/AgentMessage';
import { AgentInput } from '@app/features/block-agent/ui/AgentInput';
import '@app/features/agents-view/agents-view.css';
import { ViewShell } from '@app/components/view-shell/ViewShell';
import { Message } from '@channel/Message/Message';
import { NavGlyph, type NavIcon } from '@components/app/sidebar-next/nav-glyph';
...
/** Real Markdown editor and composer surfaces, with only local send handlers. */
function DemoComposer(props: {
  ...
  const config = buildConfig('chat')
    .namespace(`workspace-${props.label}`)
    .withHistory()
    .withCode()
    ...
    <div class="workspace-demo-composer">
      <ComposerSurface as="div" class="flex items-end gap-2 rounded-3xl p-2.5">
        ...
        <MarkdownShell
          ...
          class="min-w-0 flex-1 max-h-28 h-auto text-sm [&_[data-markdown-editable]]:outline-none [&_[data-markdown-editable]>.md-p]:my-0"
        />
  ...
  return (
    <Message.Root
      ...
      message={{
        ...
        sender_id: `demo-${props.message.person}`,
        created_at: '',
        updated_at: '',
        attachments: [],
        reactions: [],
    ...
    >
      <Message.Layout>
        ...
        <Message.Slot placement="content">
          <Message.Content class="text-sm" />
        </Message.Slot>
        <Message.Slot placement="footer">
          <button
            type="button"
...
function DocumentDemo(props: { demo: WorkspaceDemo }) {
  const editor = buildConfig('markdown')
    .namespace('workspace-launch-document')
    .withHistory()
    .withCode()
    .withLinks({ floatingMenu: true })
    .withFloatingFormatMenu()
    .withSkipPreviewFetch()
    .onChange(props.demo.setDocument);
  return (
    <div class="workspace-demo-scroll workspace-demo-document">
      <h1 class="mb-6 text-2xl font-semibold">Q3 launch plan</h1>
      <MarkdownShell
        config={editor}
        initialValue={props.demo.document()}
        class="h-auto text-sm"
        refFn={(element) => {
          element.setAttribute('aria-label', 'Q3 launch plan document');
          element.setAttribute('role', 'textbox');
        }}
      />
    </div>
    ...
    <>
      ...
      <div class="workspace-demo-composer">
        <AgentInput
          ...
          autofocus={false}
          placeholder={
            coding() ? 'Ask about the deploy fix' : 'Ask about the launch'
          }
          onSend={props.demo.ask}
          disabled={props.demo.busy()}
          modelControl={
            <span class="text-xs text-ink-muted">
              {coding() ? 'Auto' : 'Sonnet 5'} ⌄
            </span>
          }
        />
      </div>
