...
import IconPlus from '@lucide/plus.svg';
import { cn } from '@ui';
...
export function ThreadReplyButton(props: ThreadReplyButtonProps) {
  ...
  return (
    <button
      type="button"
      class={cn(
        'rounded-full icon-plus mb-2 border border-thread-rail hover:bg-hover flex flex-row justify-center items-center size-8 touch:min-h-(--user-icon-width) touch:min-w-(--user-icon-width) text-ink-muted hover:text-ink',
        local.class
      )}
      ref={(el) => {
