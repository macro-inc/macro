...
/**
 * V4's app chrome: V3's top bar stood on its left edge. Create and the live
 * search sit at the top, the primary views run down the middle with the
 * filled-icon active state and an accent dot on the row's left edge, and the
 * bottom cluster opens the companion splits and
...
export function ExperimentalAppSidebar(props: ExperimentalAppSidebarProps) {
  ...
  return (
    <aside
      ...
      class={cn(
        'relative flex h-full shrink-0 flex-col gap-[9px] overflow-hidden border-r border-edge-muted bg-surface pb-3 pt-2',
    ...
    >
      ...
      >
        <SidebarCreateMenu
          ...
          filled
          large
          triggerClass="glass size-10! border-transparent bg-ink/5 px-0 text-ink hover:bg-ink/10 hover:text-ink!"
        ...
        fallback={
          ...
          >
            <button
              ...
              class={cn(
                'group/side-view relative flex h-10 w-full items-center rounded-lg font-medium outline-none transition-colors text-ink-muted hover:bg-ink/5 hover:text-ink focus-visible:ring-2 focus-visible:ring-accent/40',
                isSlim() ? 'justify-center' : 'gap-3 px-3 text-left'
              )}
              {...pressHandlers(chrome.openSearch)}
            >
              <span class="relative flex size-6.5 shrink-0 items-center justify-center">
                <SearchIcon class="size-6" />
              </span>
              <Show when={!isSlim()}>
                <span class="min-w-0 flex-1 truncate">Search Macro</span>
              </Show>
            </button>
          </Tooltip>
        }
      ...
      <nav aria-label="App views" class="mt-[5px] min-h-0 flex-1 overflow-y-auto">
                  ...
                  >
                    ...
                    >
                      <Show when={chrome.isActive(destination)}>
                        <span class="pointer-events-none absolute left-0 top-1/2 size-1.5 -translate-y-1/2 rounded-full bg-accent" />
                      </Show>
                      <span class="relative flex size-6.5 shrink-0 items-center justify-center">
                        <Dynamic
                          component={
                        ...
                        <Show
                          when={
                            isSlim() && chrome.unreadCount(destination) > 0
                          }
                        >
                          <span
                          ...
                          >
                            {unreadBadgeLabel(
                              chrome.unreadCount(destination)
                            )}
                          </span>
                        </Show>
                      </span>
      ...
      <footer
        class={cn(
          'flex shrink-0 items-center gap-[7px] border-t border-edge-muted/70 pt-3',
