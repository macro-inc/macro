...
/** Structural row contract used by entity actions across list implementations. */
export type EntityActionListRow = {
  identityKey: string;
  id: string;
  index: number;
  original: EntityData;
  group: { isExpanded: () => boolean } | undefined;
...
type EntityActionNavigationOptions = {
  wrapNavigation?: boolean;
  skipGroupHeaders?: boolean;
  skipLoadMore?: boolean;
  skip?: (row: EntityActionListRow) => boolean;
};

type EntityActionNavigationResult =
  | { row: EntityActionListRow; index: number }
  | undefined;

/** List capabilities used by entity actions, independent of a concrete view. */
export type EntityActionListState = {
  focus: {
    id: () => string | undefined;
    index: () => number;
    set: (id: string | undefined) => unknown;
  };
  navigate: {
    peekOffset: (
      offset: number,
      options?: EntityActionNavigationOptions
    ) => EntityActionNavigationResult;
  };
  items: {
    count: () => number;
    get: (id: string) => EntityActionListRow | undefined;
    at: (index: number) => EntityActionListRow | undefined;
  };
  selection: { clear: () => void };
  collapseEntity: {
    callback: () => ((entityId: string) => Promise<void>) | undefined;
...
/**
 * Adapts a generic list controller to the narrow state consumed by entity
 * actions.
 */
export function toEntityActionListState<TItem, TMetadata>(
  ...
  const peekOffset: EntityActionListState['navigate']['peekOffset'] = (
    ...
    const shouldSkip = (row: EntityActionListRow) =>
