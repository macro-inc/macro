...
export const makeDeleteAction = (options: MakeDeleteOptions) => {
  ...
  const executeWithSoup = async (
    ...
    const isFocusableRow = (
      row: EntityActionListRow | undefined
    ): row is EntityActionListRow =>
      row !== undefined &&
      !row.getIsGrouped() &&
      !row.getIsLoadMore() &&
      (!row.group || row.group.isExpanded());
    const captureAnchor = (row: EntityActionListRow | undefined) =>
      isFocusableRow(row)
    ...
    const anchorNavigation = {
      ...
      skipLoadMore: true,
      skip: (row: EntityActionListRow) => requestedIds.has(row.original.id),
    };
