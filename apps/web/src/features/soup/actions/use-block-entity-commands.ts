...
import { openEntityInSplitFromUnifiedList, restoreSoupFocus } from '../utils';
import type {
...
export type UseBlockEntityCommandsOptions = {
  ...
  onDeleted?: () => void;
  /** Optional list backing the block when it is being triaged from a list. */
...
export const useBlockEntityCommands = (
  ...
  const openPropertyEditorIfSelected = (
    ...
    if (entity) {
      openPropertyEditor([entity], mode, property, {
        restoreFocus: () => {
          if (list) return restoreSoupFocus(entity.id);
        },
      });
    }
  };
  ...
  /**
   * The list row for the entity when this block is being triaged out of a list,
   * which is what decides between advancing the split and leaving it be.
   */
  const triageRow = () => {
    const entity = getEntity();
    if (!entity || !canUseMarkDoneHotkey() || !list) return undefined;
    return list.items.get(entity.id);
  };
  ...
  const runCreateReminder = () => {
    ...
    // Driven through the list when the block is being triaged out of it. Having
    ...
    if (list && selectedRow) {
      void createReminderAction.executeWithSoup([selectedRow.original], list, {
        advances: true,
      });
      return true;
    }
  ...
  const runMarkDone = () => {
    ...
    // triage flow, or the entity is no longer in the surviving list.
    const selectedRow = triageRow();
    if (!list || !selectedRow) {
    ...
    markDone.executeWithSoup([selectedRow.original], list, advanceSplitTo);
