...
import { nextNotificationState, notificationStateFromGraphql, notificationStatesForFilter } from '../notification-state';
...
describe('notification filter intent', () => {
  it('preserves the complete domain transition table', () => {
    for (const before of ['unseen', 'seen', 'done'] as const) {
      expect(nextNotificationState(before, 'MARK_SEEN')).toBe(before === 'unseen' ? 'seen' : before);
      expect(nextNotificationState(before, 'MARK_DONE')).toBe('done');
      expect(nextNotificationState(before, 'MARK_UNDONE')).toBe(before === 'done' ? 'seen' : before);
    }
