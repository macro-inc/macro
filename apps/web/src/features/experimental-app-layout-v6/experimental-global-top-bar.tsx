...
import {
  ...
  mockInboxNotification,
  setMockAgentLoading,
  setMockAgentNotification,
  setMockInboxNotification,
...
const DESTINATION_ID_BY_CONTENT_ID = new Map(
  [...LEFT_DESTINATIONS, ACTIVITY_DESTINATION].map((destination) => [
    destination.contentId,
    destination.id,
  ])
);
const rememberedDestinationViews = new Map<
  TopBarDestination['id'],
  SplitContent
>();

function defaultDestinationContent(
...
function rememberDestinationViews() {
  const handle = globalSplitManager()?.activeSplit();
  if (!handle) return;
  handle.captureEntryState();
  for (const content of handle.history()) {
    if (content.type !== 'component') continue;
    const destinationId = DESTINATION_ID_BY_CONTENT_ID.get(content.id);
    if (destinationId) rememberedDestinationViews.set(destinationId, content);
  }
}
...
/** Top-bar-only global navigation used by Experimental v6. */
export function ExperimentalGlobalTopBar() {
  ...
  const openDestination = (
    ...
    layout.openWithSplit(
      !event.shiftKey
        ? (rememberedDestinationViews.get(destination.id) ??
            defaultDestinationContent(destination))
        : defaultDestinationContent(destination),
      {
  ...
  return (
    <header class="flex min-w-0 shrink-0 items-center gap-2 bg-page p-2">
      ...
      <nav aria-label="Global views" class="flex shrink-0 items-center gap-0.5">
        ...
        <ExperimentalInboxPopover
          hasMockNotification={mockInboxNotification()}
        />
        <span
          aria-hidden="true"
      ...
      <div class="fixed bottom-12 right-4 z-action-menu">
          ...
          <Dropdown.Content class="min-w-52 shadow-menu">
            <Dropdown.Group>
              <Dropdown.Item
                ...
                onSelect={() =>
                  setMockInboxNotification((value) => !value)
                }
              >
                <span
                  class={cn(
                    'size-2 rounded-full border border-edge-muted',
                    mockInboxNotification() && 'border-accent bg-accent'
                  )}
                />
                <span>Inbox notification</span>
              </Dropdown.Item>
              <Dropdown.Item
                class="flex cursor-default items-center gap-2 px-2.5 py-2 text-sm text-ink outline-none"
                onSelect={() =>
                  setMockAgentNotification((value) => !value)
                }
              >
                <span
                  class={cn(
                    'size-2 rounded-full border border-edge-muted',
                    mockAgentNotification() && 'border-accent bg-accent'
                  )}
                />
              ...
              <Dropdown.Item
                class="flex cursor-default items-center gap-2 px-2.5 py-2 text-sm text-ink outline-none"
                onSelect={() => setMockAgentLoading((value) => !value)}
              >
                <span
                  class={cn(
                    'size-2 rounded-full border border-edge-muted',
                    mockAgentLoading() && 'border-accent bg-accent'
                  )}
                />
