...
const TASK_PERSONAL_ITEMS: readonly ViewNavigationItem[] = [
  { value: 'my-tasks', label: 'My tasks', icon: CircleBoldIcon },
...
const TASK_TEAM_ITEMS: readonly ViewNavigationItem[] = [
  { value: 'team-tasks', label: 'Team tasks', icon: CircleBoldIcon },
];
...
const MACHINE_ITEMS: readonly (ViewNavigationItem & { value: PowersTab })[] = [
  { value: 'agents', label: 'Agents', icon: LaptopIcon },
  ...
  const BrainNavigation = () => (
    <div class="flex min-h-0 flex-1 flex-col">
      <ExperimentalViewSidebarItems class="shrink-0">
        <nav aria-label="Agents sections" class="flex flex-col gap-0.5">
      ...
      <>
        <Show when={powersDetail()}>
          <button
            ...
            aria-label="Close Agents details"
    ...
    >
      <div class="flex size-full min-h-0">
        <ExperimentalViewSidebar
          ...
          class="mb-0 border-r-0!"
          collapsed={viewSidebarCollapsed()}
        >
          <ComposedSplitHeader class="flex min-h-10 shrink-0 items-center gap-2">
        ...
        <ListContentContainer class="bg-ink/2">
              ...
              {(tag) => {
                ...
                return (
                  <button
                    type="button"
                    class={cn(
                      'experimental-v6-drive-tag inline-flex min-w-0 max-w-full items-center rounded-full px-2 py-1 text-xs transition-[filter,background-color,color]',
                      active()
  ...
  const MessagesLayout = () => (
    <div class="flex size-full min-h-0">
      ...
      <div class="min-h-0 min-w-0 flex-1">
        <div class="size-full min-h-0 overflow-hidden bg-ink/2">
          <Show
