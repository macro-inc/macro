    ...
    <Show when={show() && domSelection()}>
      {(selection) => (
        <ScopedPortal scope="block">
          <button
            type="button"
            class="fixed top-0 left-0 z-highlight-menu flex items-center gap-1.5 rounded-full border border-edge bg-surface px-2.5 py-1 text-xs font-medium text-ink shadow-lg transition-[filter] duration-150 hover:brightness-90"
