...
describe('activityLabel', () => {
  it('narrates the create, which blocks on the sandbox boot', () => {
    expect(activityLabel(facts({ pending: true }))).toBe(
  ...
  it('narrates a resume, over anything the stale transcript says', () => {
    ...
    ).toBe("Waking the agent's sandbox…");
  });
  ...
  it('still reports the sandbox when a prompt beat it there', () => {
    expect(
      activityLabel(
        facts({
          working: true,
    ...
    ).toBe("Starting the agent's sandbox…");
  });
