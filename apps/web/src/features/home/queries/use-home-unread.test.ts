...
import { useHomeEntitiesQuery } from './use-home-query';
...
function setup(tab: HomeTab = 'signal') {
  return createRoot((cleanup) => {
    dispose = cleanup;
    return useHomeEntitiesQuery({ tab, facets: {} });
  });
}
...
describe('Home unread presence', () => {
  ...
  it.each(['array', 'accessor', 'global'] as const)(
    ...
    (attachment) => {
      ...
      createRoot((cleanup) => {
        dispose = cleanup;
        const inbox = useHomeEntitiesQuery({ tab: 'signal', facets: {} });
