...
export type HomeQueryCapabilities = {
  calendar: boolean;
 ...
 */
export const homeTabIsNotificationFeed = (tab: HomeTab): boolean =>
 ...
 * see `homeGroupTimestamp` — so a live notification still re-buckets its row
 * even while the server keeps sorting by content recency.
 */
export const homeTabOrdersByNotification = (
  context: Pick<HomeViewContext, 'tab' | 'capabilities'>
): boolean =>
  context.capabilities.notifiedSort && homeTabIsNotificationFeed(context.tab);

/** Builds the heterogeneous Soup AST for the composable Home view. */
