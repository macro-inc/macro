...
import { type HomeViewContext, homeTabIsNotificationFeed } from './home-query';

type HomeOrderContext = Pick<HomeViewContext, 'tab' | 'capabilities'>;

export const homeSortTimestamp = (entity: EntityData) =>
 ...
 */
export const homeGroupTimestamp = (
  entity: EntityData,
  context: HomeOrderContext
) =>
  (homeTabIsNotificationFeed(context.tab) ? entity.notifiedAt : undefined) ??
  homeSortTimestamp(entity);

export function groupHomeEntitiesByTabDate<T extends EntityData>(
  entities: T[],
  context: HomeOrderContext,
  now = new Date(),
  getTimestamp: (entity: EntityData) => DateValue | null | undefined = (
    entity
  ) => homeGroupTimestamp(entity, context)
): SoupGroup<T>[] {
  ...
  return deduplicateItems<EntityWithRawNotifications<EntityData>>(
    [
      ...notifications.map((entity) => ({
        ...entity,
        sortTs: homeGroupTimestamp(entity, context),
      })),
