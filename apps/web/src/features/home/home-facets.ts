...
import type { HomeReadFilter, HomeTypeFilter } from './types';

export type HomeFacetContext = {
  notificationSource?: NotificationSource;
};

export type HomeFacetOption = FacetOption<EntityData, HomeFacetContext> & {
  label: string;
};
...
const typeOptions: Record<HomeTypeFilter, HomeFacetOption> = {
  documents: {
    id: 'documents',
    label: 'Documents',
    clause: {
...
type HomeFilterGroup = {
  id: string;
  label: string;
  options: { id: string; label: string }[];
};

export const HOME_TYPE_FILTER_GROUPS: HomeFilterGroup[] = [
  {
    id: 'type',
    label: 'Entity types',
