...
/** Places a flat 24×24 brand logo onto a module's face. */
export const LOGO_FACE_TRANSFORM =
  createOrthoPerspectiveTransform(LOGO_FACE_PLACEMENT);
 ...
 * settle back. Pass `autoAccelerateMs` to also fire that burst on a timer.
 *
...
export function Module(props: {
  ...
  state?: ModuleState;
  /** Also fire the acceleration burst on this interval (ms), in addition to click. */
  autoAccelerateMs?: number;
  /** Delay before the first auto burst (default 0). Used with `autoAccelerateMs`. */
  autoAccelerateDelayMs?: number;
  /** When set, the module acts as a link (role=link; no HTML <a>, which breaks SVG). */
  href?: string;
  ariaLabel?: string;
  ...
  let autoDelay: ReturnType<typeof setTimeout> | undefined;
  ...
  // The click / auto "acceleration" burst. Ring scale + dash composite onto the
  ...
  onMount(() => {
    ...
    // Optional auto burst: same motion as click, on a repeating timer.
    const autoMs = props.autoAccelerateMs;
    if (autoMs && autoMs > 0) {
      autoDelay = setTimeout(() => {
        accelerate();
        autoInterval = setInterval(accelerate, autoMs);
      }, props.autoAccelerateDelayMs ?? 0);
    }
  });
  ...
  return (
    <g
      ref={root}
      class={`module-bob ${props.href ? 'module-link ' : ''}${props.class ?? ''}`}
