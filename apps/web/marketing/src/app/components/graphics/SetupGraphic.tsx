/*
 * Ported from the Macro app: apps/web/src/features/setup/SetupGraphic.tsx, off
 * branch aidan/onboarding-v4-graphics (at c3129038f), where it's the /setup
 * onboarding empty state. Here it's the /migrate hero.
 *
 * Kept as close to its source as possible so the two stay diffable. The app's
 * semantic color tokens are bridged onto this site's palette in the CSS; the JS
 * needs no bridge, since upstream now reads the a0/c0 theme signals directly
 * (see useReactiveColorString, which this site mirrors).
 *
 * Site-only additions, with no upstream counterpart: the `importButton` CTA
 * below the blocks, and the auto-demo cycle (AUTO_CYCLE_MS) that fires the
 * GitHub/Notion bursts and the card slide on a loop.
 *
 * Deliberately not carried over: the app's `.logo-ref` group — invisible
 * geometry that exists only so its BrandHandoff animation can measure where the
 * real logo belongs. Nothing on this site does that handoff.
 */

import { createEffect, For, on, onCleanup, onMount, Show } from 'solid-js';
import { useReactiveColorString } from '../../../lib/theme/signals/themeReactive';
...
function DownloadPulse(props: { d: string }) {
  // The `typeof window` guard is site-only: upstream reads matchMedia straight
  // off `window` here, which is safe in the client-only app but throws under
  // this site's build-time prerender (no window). Currently unreachable during
  // SSR — nothing puts a module in `downloading` — but it would be a build
  // crash, not a runtime one, the first time something did.
  if (
...
/** Bottom import CTA geometry. The button and its connector use the viewport
 * centre rather than the (slightly off-centre) block assembly, so they sit in
 * the middle of the card. */
const [SCENE_MIN_X, , SCENE_WIDTH] = SCENE.viewBox.split(' ').map(Number);
const IMPORT_CENTER_X = SCENE_MIN_X + SCENE_WIDTH / 2;
// Start inside the middle card instead of exactly on its lower outline, so the
// dashed run stays visibly connected after the Supermodule is centred
// independently.
const IMPORT_SUPERMODULE_TOP = 774;
// The Supermodule's top cylinder is its handoff point. Its body projects down
// and right from there, so its visual centre cannot share the connector's x/y.
const IMPORT_SUPERMODULE_CENTER_X = IMPORT_CENTER_X - 144;
const IMPORT_CTA_COPY = {
  // These are scene coordinates, intentionally outside the moving module
  // group so the copy stays anchored while the capsule bobs.
  importLabel: point(576, 918),
  oneClick: point(262, 804),
} as const;

const importConnector = (verticalOffset = 0) =>
  `M${IMPORT_CENTER_X} 600 L${IMPORT_CENTER_X} ${IMPORT_SUPERMODULE_TOP + verticalOffset + 22}`;
const importSupermoduleTransform = (verticalOffset = 0) =>
  `translate(${IMPORT_SUPERMODULE_CENTER_X} ${730 + verticalOffset}) scale(0.58) translate(-363 -323)`;

/** The scene frame, extended downward so the import button fits below the
 * blocks. Derived from
 *  SCENE.viewBox so upstream reframing the scene horizontally carries over —
 *  only the height is overridden. */
const importViewBox = (verticalOffset = 0) => {
  const [minX, minY, width] = SCENE.viewBox.split(' ');
  // The SVG's layout box finishes at the Supermodule's centre. With the root's
  // visible overflow, its lower edge projects beyond the card slightly.
  const height = 1008 + verticalOffset - Number(minY);
 ...
 *  connector. Optional `autoAccelerate*` mirrors {@link Module}'s timer burst. */
export type ModuleConfig = {
  slot: SlotName;
  logo: ModuleLogo;
  connector?: boolean;
  state?: ModuleState;
  autoAccelerateMs?: number;
  autoAccelerateDelayMs?: number;
};

/** Shared period for the hero's auto demo: GitHub / Notion bursts and the
 *  Macro card slide, staggered so they don't overlap. Click still works. */
const AUTO_CYCLE_MS = 9000;

/** Delay before the card slide joins the auto cycle — after the two module
 *  bursts below, so the three don't pile up. */
const SLIDE_CYCLE_DELAY_MS = 8000;
 ...
 *  GitHub and Notion also auto-fire their click burst on a staggered timer. */
const DEFAULT_MODULES: ModuleConfig[] = [
  { slot: 'frontPill', logo: MODULE_LOGOS.Linear },
  { slot: 'midBack', logo: MODULE_LOGOS.Google },
  {
    slot: 'midPort',
    logo: MODULE_LOGOS.GitHub,
    autoAccelerateMs: AUTO_CYCLE_MS,
    autoAccelerateDelayMs: 2000,
  },
  {
    slot: 'midFar',
    logo: MODULE_LOGOS.Notion,
    autoAccelerateMs: AUTO_CYCLE_MS,
    autoAccelerateDelayMs: 5000,
  },
...
/**
 * The isometric connectors scene (the Macro app's `/setup` empty state; the
 * `/migrate` hero here): the Macro logo re-imagined as three technical
 * isometric blocks (`front_card`, `middle_card`, `back_card`), with hovering
 * modules around it. Inlined (rather than imported as an asset) so animation
 * can target the named groups.
 *
 ...
 *
 * The back card's front section (`animating_card`) slides out and back on a
 * timer (and on click). When it settles, the inset LED indicators illuminate
 * bottom-to-top and hold a faint pulsing glow with a breathing halo; a fresh
 * slide darkens them until it settles again. GitHub and Notion modules also
 * auto-fire their acceleration burst on the same cycle, staggered.
 */
export function SetupGraphic(props: {
  class?: string;
  modules?: ModuleConfig[];
  /** Adds the bottom "Import via MCP with 1 click" CTA under the Macro blocks.
  ...
  importButton?: {
    ...
    /** Moves the capsule, its labels, and the connector endpoint lower in the
     * SVG scene while keeping the connector attached to the middle card. */
  ...
  const importVerticalOffset = () => props.importButton?.verticalOffset ?? 0;
  ...
  let importCta: SVGGElement | undefined;
  let slideAnimation: Animation | undefined;
  let slideDelay: ReturnType<typeof setTimeout> | undefined;
  let slideCycleDelay: ReturnType<typeof setTimeout> | undefined;
  ...
  onMount(() => {
    if (svg && importCta && props.importButton?.horizontalOffsetPx) {
      const positionImportCta = () => {
        // SVG group transforms use viewBox units, whereas the scene's visual
        // nudge is in CSS pixels. Convert the requested correction on every
        // resize so the CTA can remain physically centred.
        const scale = svg.clientWidth / SCENE_WIDTH;
        if (scale > 0) {
          importCta!.setAttribute(
            'transform',
            `translate(${props.importButton!.horizontalOffsetPx! / scale} 0)`
          );
        }
      };
      positionImportCta();
      const importCtaResizeObserver = new ResizeObserver(positionImportCta);
      importCtaResizeObserver.observe(svg);
      onCleanup(() => importCtaResizeObserver.disconnect());
    }
    ...
    // Macro card slide: a quick intro play, then the same AUTO_CYCLE_MS loop as
    // GitHub / Notion, joining late so it sits after their bursts.
    slideDelay = setTimeout(playSlide, MOTION.cardSlide.delayMs);
    slideCycleDelay = setTimeout(() => {
      playSlide();
      slideInterval = setInterval(playSlide, AUTO_CYCLE_MS);
    }, SLIDE_CYCLE_DELAY_MS);
  });
  ...
  return (
    <svg
      ...
      // modules near the top edge and the blocks near the bottom. Extra height
      // when the import CTA is present so the bottom module + labels fit.
      viewBox={
        props.importButton
          ? importViewBox(importVerticalOffset())
          : SCENE.viewBox
      }
      fill="currentColor"
      class={`setup-graphic ${props.poweredUp ? 'powered ' : ''}${props.class ?? ''}`}
      onClick={playSlide}
      // Decorative on its own, but the import CTA inside is a real link, so the
      // subtree has to stay reachable when it's present.
      aria-hidden={props.importButton ? undefined : 'true'}
    >
      <g class="scene">
          ...
          <For each={modules()}>
            {(module, index) => (
              <Module
                ...
                autoAccelerateMs={module.autoAccelerateMs}
                autoAccelerateDelayMs={module.autoAccelerateDelayMs}
              />
            )}
          </For>
        </g>
        <Show when={props.importButton}>
          {(button) => (
            <g ref={importCta} class="import-cta">
              <Supermodule
              ...
              <path
                class="connector"
                d={importConnector(importVerticalOffset())}
              />
            </g>
          )}
        </Show>
