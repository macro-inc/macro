...
// Ported from the app's `@icon/wide-star` AnimatedStarIcon — the same mark the
// product sidebar uses for Agents. The inner rhombus/eye inflate while the
// surrounding `.more-features-pill` is hovered, mirroring the sidebar's
// hover animation (no JS needed; driven purely off the ancestor :hover).
export function AgentsStarIcon(props: {
  ...
  return (
    <svg
      ...
      stroke="none"
      overflow="visible"
      class={`agents-star-icon${props.class ? ` ${props.class}` : ''}`}
      style={props.style}
      xmlns="http://www.w3.org/2000/svg"
    >
      <style>{`
        .agents-star-icon .star-inflate,
        .agents-star-icon .star-eye,
        .agents-star-icon .inflated-rhombus {
          transform-origin: 9px 6px;
          transition: transform 0.4s ease;
        }
        @media (hover) {
          .more-features-pill:hover .agents-star-icon .star-inflate,
          .more-features-pill:hover .agents-star-icon .inflated-rhombus {
            transform: scale(2.22);
          }
          .more-features-pill:hover .agents-star-icon .star-eye {
            transform: scale(2);
          }
        }
      `}</style>
