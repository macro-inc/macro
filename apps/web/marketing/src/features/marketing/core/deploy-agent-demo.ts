/** Website-owned fixture types; intentionally separate from the app protocol. */
export type DemoToolDetail =
...
export type DemoMessagePart =
  ...
  | {
      kind: 'tool_use';
      id: string;
      name: { kind: 'native'; name: string };
      status: 'completed';
      detail: DemoToolDetail;
    };
export type DemoAgentMessage = {
  ...
  requestId: null;
  pending: boolean;
  author: { kind: 'agent' } | { kind: 'user'; userId: string };
  stop: { kind: 'end_turn' } | null;
...
export const DEPLOY_TRACE: DemoAgentMessage = {
