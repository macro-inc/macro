...
import type { ThemeReactive, ThemeV1Tokens } from '../types/themeTypes';
...
/**
 * A token's current color as a concrete oklch() string, tracked reactively.
 *
 * For code that can't hand a `var(--a0)` to the browser and let the cascade do
 * the work — WAAPI keyframes, in particular, won't interpolate a custom
 * property, and reading one back with getComputedStyle returns its
 * unsubstituted text. Because this is a signal, colors re-read on a theme
 * change instead of staying baked at whatever they were when the animation
 * started.
 *
 * Mirrors the same helper in the Macro app (features/theme/signals), so the
 * graphics ported from there can use it unchanged.
 */
