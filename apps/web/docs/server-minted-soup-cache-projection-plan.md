...
- [x] Phase 1: server projection-source hydration. The flat expanded cursor, frecency fallback, and bounded by-ID paths now return authorized item/source pairs from one SQL result; the document relation probe uses `document_email_pkey`. Root-level `just prepare_db` succeeds, and `cargo test -p soup --all-features` passes all 229 enabled tests (2 ignored), including database-backed projection hydration and index-plan coverage.
