...
native reachability notifications. Dropdown filters (read/unread, done, etc.),
app restart/persistence, offline pagination, multiple accounts and iOS
reachability are follow-up scenarios. The initial test changes the real Mail
tabs, which change Soup's importance predicate and INBOX/ALL scope.
