...
Macro supports [multiple email accounts in one inbox](/product/email#multi-account-inbox) and multiple email calendar views. **Each account needs calendar enabled on its own.** Every linked inbox in **Settings → Connections** carries its own **Enable calendar** button.
...
Details are edited inline on the event itself. Title and description are typed directly, and everything else is a pill you click to open a small editor in place:
...
By default Macro looks at working hours of 9:00 to 18:00 and skips weekends. Adjust both from the settings control next to the button.
...
The same tools are available over Macro's MCP server:
...
Macro deletes its copy of that account's events and gives up calendar access. Your Google Calendar is untouched and the inbox keeps syncing mail as normal. Turning calendar back on means granting access again.
