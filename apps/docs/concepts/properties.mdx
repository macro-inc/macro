...
Agents read and write properties through the MCP entity tools: `GetEntityProperties` to read, `SetEntityProperty` to update (status, assignee, dates, custom fields), `ListTasks` to query tasks by those fields, and `ListEntities` to browse the mixed workspace feed. See the [tool reference](/AI/mcp/tools/index).
