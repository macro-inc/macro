...
You can read and write properties through the MCP entity tools: `GetEntityProperties` to read, `SetEntityProperty` to update (status, assignee, dates, custom fields), `CreateCustomProperty` to add a new custom field, and `ListEntities` to browse. See the [tool reference](/AI/mcp/tools/index).
