...
| --- | --- | --- | --- |
| `sender_email` | string | Yes | The sender's email address, e.g. `from.email` on a GetThread message or `senderEmail` on a QuerySoup email thread. Exact address, not a domain. |
| `policy` | `"signal"` \| `"noise"` \| `"block"` | Yes | Where future mail from this sender lands: `signal`, `noise`, or `block`. |
| `thread_id` | string | No | Apply the policy to the inbox that owns this thread (same UUID returned by QuerySoup, search, or GetThread). Takes precedence over `inbox`. |
