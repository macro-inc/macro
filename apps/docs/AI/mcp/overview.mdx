...
MCP access is available on every Macro plan, including Free.
...
```bash
grok mcp add --transport http macro https://mcp-server.macro.com/mcp
```

Start Grok Build, open `/mcps`, select **macro**, press **`i`** to authenticate, and complete the browser sign-in flow when prompted.
