...
Markdown documents, Loro snapshots, operation history, node IDs, and editor version
numbers are unchanged. A comment mark's stable `ids` entry matches the new thread's
...
3. Pause document/comment/channel/email writers and drain their in-flight requests,
   background jobs, and notification delivery. Require clients to refresh for the
...
```sh
bun scripts/message-cutover/run.ts prepare --writers-paused
bun scripts/message-cutover/run.ts finish --writers-paused
```
...
- Two browser sessions see posts, edits, reactions, and typing. Comments generate
  contextual comment notifications. An unshared email mention neither grants access
  nor delivers its content to the mentioned user.
