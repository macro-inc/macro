...
describe('shared message API contracts', () => {
  ...
  test('catch-up reads bound the timeline by activity time', async () => {
    const after = new Date('2026-09-09T11:00:00Z');
    serve((request) => {
      const url = new URL(request.url);
      const selection = JSON.parse(url.searchParams.get('selection') ?? '{}');
      expect(selection.activity_after).toBe(after.toISOString());
      expect(selection.limit).toBe(20);
      return Response.json({
        items: [listItem(thread(message()))],
        next_cursor: null,
      });
    });
    const ids = [];
    for await (const item of macro()
      .channels.byId(channelId)
      .messagesAfter(after, { pageSize: 20 })) {
      ids.push(item.id);
    }
  ...
  test('document comments keep UUIDs and rich mentions through creation, editing, and replies', async () => {
    ...
    const reply = await comment.reply('A reply');
