...
export class Agent extends PropertiedEntity<GetAgentResponse> {
  ...
  /** Provision a fresh dial-in WebSocket URL for this agent's runtime. */
  async provisionRuntimeConnection(): Promise<string> {
    const { url } = unwrap(
      await this.client.agentProxy.provisionRuntimeConnection({
        path: { agent_id: this.id },
      }),
    );
    return url;
