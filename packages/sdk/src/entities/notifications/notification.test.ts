...
const client = new Proxy({} as MacroClient, {
  get() { throw new Error('Seeded state access must not fetch'); },
});
    ...
    test(`${state} is authoritative regardless of viewing timestamp`, async () => {
      for (const viewed_at of [null, '2020-01-02T00:00:00Z']) {
        const record: ApiUserNotification = {
          id: 'notification-1', entity_id: 'channel-1', entity_type: 'channel',
          owner_id: 'macro|user@example.com', notification_event_type: 'channel_invite',
          notification_metadata: { tag: 'channel_invite', content: { invitedBy: 'macro|other@example.com' } },
          created_at: '2020-01-01T00:00:00Z', updated_at: '2020-01-01T00:00:00Z',
          sent: true, state, viewed_at,
        };
