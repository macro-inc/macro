# Apache Kafka 3.9.1 (KRaft), same pin the stack previously pulled from Docker Hub.
#
# The image is the official Apache tarball plus Nixpkgs JRE — not a registry
# base. Named-instance advertised listeners still come from compose env.
{ pkgs }:
let
  ...
  kafka = pkgs.runCommand "kafka-3.9.1" {
    nativeBuildInputs = [
      pkgs.gnutar
      pkgs.gzip
    ];
  ...
  entrypoint = pkgs.writeShellScriptBin "kafka-entrypoint" ''
    ...
    export PATH="${jre}/bin:${kafka}/bin:$PATH"
    ...
    advertised.listeners=''${KAFKA_ADVERTISED_LISTENERS:-PLAINTEXT://kafka:29092,PLAINTEXT_HOST://localhost:9092}
