...
#[tracing::instrument(skip(ctx, ip_context, db_permissions), fields(client_ip=%ip_context, user_id=%crate::api::required_user(&db_permissions.authorization.authorization).user_context.user_id, fusion_user_id=%crate::api::required_user(&db_permissions.authorization.authorization).user_context.fusion_user_id), err)]
pub async fn init_gmail_link_handler(
    ...
    enforce_inbox_paywall(
        ...
        || {
            count_accessible_email_inboxes(
                &ctx.db,
