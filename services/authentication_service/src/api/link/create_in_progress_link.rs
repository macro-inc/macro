...
#[tracing::instrument(skip(ctx, ip_context, auth), fields(client_ip=%ip_context, user_id=%auth.macro_user_id, fusion_user_id=%auth.user_context.fusion_user_id), err(Debug))]
