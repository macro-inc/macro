...
pub async fn handler(
    ...
    let links = ctx
            .auth_client
            .get_links(&auth.user_context.fusion_user_id, Some(idp_id.clone()))
            .await
            .map_err(|e| {
                tracing::error!(error=?e, "error fetching links for userid {} and idp id {}", auth.user_context.fusion_user_id, idp_id.as_str());
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ErrorResponse {
