...
#[tracing::instrument(skip(ctx, db_permissions), err, fields(user_id=%crate::api::required_user(&db_permissions.authorization.authorization).user_context.user_id))]
pub async fn handler(
    ...
    let user_id = MacroUserId::parse_from_str(
        &crate::api::required_user(&db_permissions.authorization.authorization)
            .user_context
            .user_id,
    )
