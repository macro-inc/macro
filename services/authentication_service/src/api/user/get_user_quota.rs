...
#[tracing::instrument(skip(ctx, db_permissions), fields(user_id=?crate::api::required_user(&db_permissions.authorization.authorization).user_context.user_id))]
pub async fn handler(
    State(ctx): State<ApiContext>,
    db_permissions: DbPermissionsExtractor,
) -> Result<Response, Response> {
    let user_context =
        &crate::api::required_user(&db_permissions.authorization.authorization).user_context;
