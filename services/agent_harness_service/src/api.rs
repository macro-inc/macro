...
pub async fn setup_and_serve<R, Auth>(
    control_state: AgentSessionControlState<R, Auth>,
    port: u16,
...
fn api_router<R, Auth>(control_state: AgentSessionControlState<R, Auth>) -> Router
where
    R: AgentSessionNotificationRecipient,
    Auth: MacroAuthorizationService,
{
