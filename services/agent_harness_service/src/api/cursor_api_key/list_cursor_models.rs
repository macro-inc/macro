...
use cursor_cloud_agents::domain::model::CursorModel;
...
use super::{CursorApiKeyError, CursorApiKeyState};
...
/// The models a Cursor account offers, in Cursor's order.
///
/// Shared with key registration, which seeds the new Cursor agent with the
/// first of them when the user has not chosen a default.
pub(super) async fn models_for_key(
    base_url: &str,
    key: &CursorApiKey,
) -> Result<Vec<CursorModel>, CursorApiKeyError> {
    ...
    client.list_models().await.map_err(|error| {
...
pub async fn handler<Bots: BotService, Auth: MacroAuthorizationService>(
    ...
    let models = models_for_key(&state.cursor_api_base_url, &key).await?;
