...
## Local Development
From the repository root, start the local databases with `just run_dbs`.

One time setup -- connect to the local database and run the following command:
    `CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`

From the repository root, run `just initialize_dbs` to apply the canonical
migrations from `crates/macro_db_client/migrations`.
