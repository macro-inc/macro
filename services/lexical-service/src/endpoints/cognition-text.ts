...
export class CognitionTextEndpoint extends OpenAPIRoute {
  schema = {
    ...
    description:
      'Fetches a document snapshot from the sync service and converts it to cognition text with node metadata and types using Lexical editor state parsing',
    request: {
      params: docIdParam,
    },
    responses: {
      ...
      if (rawDocument.success) {
        try {
          ...
          throw new ConversionError(
            'Failed to parse document snapshot for cognition text'
          );
        }
      } else {
