...
async def request_job(request: JobRequest) -> None:
    try:
        ...
        logger.warning("Rejecting invalid or expired voice dispatch")
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    async def publish(kind: str, message: str | None = None) -> None:
        ...
        try:
            ...
            logger.warning("Voice status could not reach caller", extra={"voice_session_id": job.voice_session_id})
    ...
    def spawn(coro) -> None:
        ...
        def complete(done: asyncio.Task) -> None:
            ...
            if not done.cancelled() and done.exception() is not None:
                logger.error("Voice background task failed", extra={"voice_session_id": job.voice_session_id})
                shutdown.set()
    ...
    try:
        ...
        logger.error("Voice session failed", extra={"voice_session_id": job.voice_session_id})
