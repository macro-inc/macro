...
use crate::{
    api::{context::ApiContext, middleware::error_handler::error_handler},
    config::Config,
};
use model::folder::UploadFolderRequest;

/// Checks if the user has hit/exceeded their document limit for a folder upload
#[tracing::instrument(skip(ctx, authorization, config_context, req, next), fields(user_id=?authorization.user_context.user_id))]
