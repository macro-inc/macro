...
- In this mode `snippets` is a **list of pairs**, not an object: `runCode({ code: "editor.setText('b3', snippets.intro)", snippets: [{ key: "intro", text: "Your composed text." }] })`. You still reference each one as `snippets.KEY` in `code`. Every key your code uses MUST appear in the list.
