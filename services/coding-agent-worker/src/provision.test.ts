...
import { ENSURE_TIMEOUT_S, assertSafeRepoUrl, ensureReady, ensureReadyCommand } from './provision'

test('accepts a plain https repo url', () => {
  expect(() => assertSafeRepoUrl('https://github.com/macro-inc/macro.git')).not.toThrow()
})
...
test('ensureReady runs the script with the ensure timeout', async () => {
  ...
  expect(calls).toEqual([{ command: ensureReadyCommand(), timeoutS: ENSURE_TIMEOUT_S }])
})
