...
use crate::pubsub::util::{
    build_notification_recipients, cg_refresh_email, publish_document_email_attachment_unlinked,
    publish_email_event,
};
...
async fn handle_delete(
    ...
    let unlinked_document_ids = email_db_client::links::delete::delete_link_by_id(&ctx.db, link.id)
        .await
        .context("Failed to delete link in background task")?;
    publish_document_email_attachment_unlinked(&ctx.macro_event_broker, unlinked_document_ids);
