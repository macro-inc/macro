...
The OpenAPI specs are located at `apps/web/src/lib/service-clients/service-<name>/openapi.json`:
...
3. Validate the service exists at `apps/web/src/lib/service-clients/service-<name>/openapi.json`
4. Create a temporary directory and HTML file to serve Swagger UI with OpenAPI 3.1.0 support:

```bash
SPEC_PATH=$(realpath apps/web/src/lib/service-clients/service-<name>/openapi.json)
TEMP_DIR=$(mktemp -d)
