...
const NextEmailContext = createContext<NextEmailContextValues>();

export function NextEmailProvider(props: FlowProps<{ threadID: string }>) {
...
export function useNextEmailContext() {
  const ctx = useContext(NextEmailContext);
  if (!ctx) {
    throw new Error(
      'useNextEmailContext must be used within an NextEmailProvider'
    );
  }
  return ctx;
}
