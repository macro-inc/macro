...
// ----- New Consolidated Filter Chips Component -----

interface ConsolidatedFilterChipsProps {
...
/**
 * Renders consolidated filter chips with the new segmented design.
 * Groups multiple values from the same category into a single chip.
 */
export const ConsolidatedFilterChips = (
	props: ConsolidatedFilterChipsProps,
) => {
	return (
		<div class="flex items-center gap-2 flex-wrap">
			<For each={props.filters}>
			...
			<Show when={props.filters.length > 0}>
				<Button onClick={() => props.onClearAll()} variant="ghost" size="sm">
					<XIcon class="size-3!" />
					Clear all
				</Button>
			</Show>

			<AddFilterButton onClick={props.onAddFilter} />
		</div>
	);
};
