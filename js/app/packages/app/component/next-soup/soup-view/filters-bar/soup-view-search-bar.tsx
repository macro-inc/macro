...
export const SoupSearchbar = (props: SoupSearchbarProps) => {
  ...
  const activeMention = createMemo(() => {
    ...
    return detectActiveMention(searchText(), cursorPos());
  });

  createEffect(() => {
    if (activeOperator() || activeMention()) {
  ...
  const handleMentionSelect = (option: AutocompleteOption) => {
    ...
    // Replace @partial with the user's email (matches what's indexed in OpenSearch)
    const email = option.id.split('|').pop() ?? option.id;
    const before = searchText().slice(0, mention.startIndex);
    const after = searchText().slice(mention.endIndex);
    setSearchText((before + email + after).replace(/\s{2,}/g, ' ').trim());
