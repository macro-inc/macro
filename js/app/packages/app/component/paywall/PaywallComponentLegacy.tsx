...
import { LEGACY_PLANS, type PaidPlanTier, PLAN_FEATURES } from './plans';
import SubscriptionTier from './SubscriptionTier';

type LegacyPaidPlan = Exclude<(typeof LEGACY_PLANS)[number], { tier: 'free' }>;

// Paid-only plans for the billing paywall — Stripe has no product for the
// 'free' tier, so it must be excluded here.
const PaywallComponentLegacy = (props: PaywallProps) => {
  const paidPlans = LEGACY_PLANS.filter(
    (p): p is LegacyPaidPlan => p.tier !== 'free'
  );
  ...
  const handleCheckout = async (tier: PaidPlanTier) => {
    try {
      ...
      const url = await stripeServiceClient.createCheckoutSession({
        ...
        tier: tier as StripeProductTier,
      });
  ...
  const handleUpdateTier = async () => {
    ...
    setUpdating(true);
    try {
      const result = await stripeServiceClient.updateSubscriptionTier(
        next as StripeProductTier
      );
      if (!result.ok) {
          ...
          fallback={
            ...
            >
              <Show when={!hasPaid()} fallback={'Manage Subscription'}>
                Get {LEGACY_PLANS.find((p) => p.tier === selectedTier())?.name}
              </Show>
            </Button>
          }
        >
...
export default PaywallComponentLegacy;
