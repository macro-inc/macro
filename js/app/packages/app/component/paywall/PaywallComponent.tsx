...
const PaywallComponent = (props: PaywallProps) => {
  ...
  return (
    <section class="relative flex w-full flex-col gap-6">
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] p-6 sm:p-8">
        <section class="flex flex-col gap-2">
          <LogoIcon class="mb-2 size-10 text-accent" />
          <h2 class="text-2xl text-ink font-medium">Upgrade to Premium</h2>
          <p class="text-sm text-ink-extra-muted">
            Unlock Macro’s full workspace: stronger agents, more context, and
            the premium tools built for teams.
          </p>
          <Show when={props.errorKey}>
            <p class="text-failure-ink text-sm">
        ...
        <section class="flex flex-col gap-4 rounded-lg bg-active p-4">
          <div class="flex flex-col">
            <h3 class="text-base font-medium text-ink">Premium features</h3>
          </div>
          <PremiumFeatures />
        </section>
      </div>
