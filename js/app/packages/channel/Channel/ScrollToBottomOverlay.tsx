...
export function ScrollToBottomOverlay(props: ScrollToBottomOverlayProps) {
  return (
    <Show when={shouldShowScrollToBottomButton(props.scrollState())}>
      <button
        type="button"
        class={cn('absolute top-4 left-1/2 -translate-x-1/2 z-10 px-3 py-1.5 text-xs bg-menu border border-edge-muted hover:bg-hover hover-transition-bg', props.class)}
        onClick={() => {
