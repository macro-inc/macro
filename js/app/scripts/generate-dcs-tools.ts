...
async function fetchAIToolsSchemas(): Promise<AiToolsResponse> {
  const endpoint = '/tools/schemas';
  const url = serviceUrl(documentCognitionBase) + endpoint;

  try {
    console.log(`Fetching AI tools schemas from: ${url}`);
    ...
    const parsed = AiToolsResponseSchema.parse(data);

    console.log(
      `Successfully fetched ${parsed.schemas.length} AI tool schemas`
    );
    return parsed.schemas;
  } catch (error) {
    console.error('Failed to fetch AI tools schemas:', error);
    throw error;
  }
}
