...
public class RestrictedPayment extends ExtractorWidget {
	
	public static final String name = "Restricted Payments";
	public static final CovenantType type = CovenantType.RESTRICTED_PAYMENTS;

	public RestrictedPayment(PDFDOM dom, Collection<Result> masterResults) {
	...
	public void run() {
		for (SSection s : this.getDom().getAllSections()) {
			if (s.getSectionTitleString().toLowerCase().contains("restricted payment")) {
