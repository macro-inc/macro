...
public class Indebtedness extends ExtractorWidget {
	
	public static final String name = "Restricted Payments";
	public static final CovenantType type = CovenantType.INDEBTEDNESS;

	public Indebtedness(PDFDOM dom, Collection<Result> masterResults) {
	...
	public void run() {
		for (SSection s : this.getDom().getAllSections()) {
			if (s.getSectionTitleString().toLowerCase().contains("indebtedness")) {
