...
function run(
  op: DocumentOp,
  opts: { randomSource?: RandomSource; docReader?: DocReader } = {}
) {
  // pin speed 400 wpm → msPerChar 30 for exact-ms assertions
  const msPerChar = 60_000 / (400 * 5);
  const steps = animate(op, {
    randomSource: opts.randomSource ?? mockRandomSource(),
...
describe('formatText animator — full flow', () => {
  it('sweeps the matched span (3 sweeps, left-anchored) then one match-based edit', () => {
    ...
    expect(action.steps).toEqual<DocumentOpStep[]>([
      ...
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 't1', span: { start: 9, end: 11 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 't1', span: { start: 7, end: 11 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 't1', span: { start: 6, end: 11 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 't1', span: { start: 4, end: 11 } },
      },
      { kind: 'pause', ms: 145 },
      {
        kind: 'edit',
  ...
  it('animates each occurrence with a pause between, then ONE edit covers all', () => {
    ...
    expect(
      action.steps.some(
        (s) => s.kind === 'awareness' && (s.x as any).node === 't2'
      )
    ).toBe(true);
  });
});
...
describe('highlight sweep count honors ranges (0–5)', () => {
  ...
  it('5 sweeps → six highlights (5 incremental + final)', () => {
    const action = run(
      ...
      {
        randomSource: mockRandomSource({
          integer: [30, 5, 50, 50, 50, 50, 50, 90],
        }),
        docReader: reader({
          locate: () => [{ node: 't1', start: 0, end: 10 }],
        }),
      }
    );
...
describe('setText animator — select-all, delete, type (full flow)', () => {
  it('selects the existing text, deletes it, then types in 3-char chunks', () => {
    ...
    expect(action.steps).toEqual<DocumentOpStep[]>([
      ...
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 'b1', span: { start: 4, end: 5 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 'b1', span: { start: 2, end: 5 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 'b1', span: { start: 1, end: 5 } },
      },
      { kind: 'pause', ms: 75 },
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 'b1', span: { start: 0, end: 5 } },
      },
      { kind: 'pause', ms: 145 },
...
describe('enriched structural animations', () => {
  ...
  it('mergeBlocks highlights each block then merges', () => {
    ...
    expect(
      steps.some((s) => s.kind === 'awareness' && (s.x as any).node === 'b1')
    ).toBe(true);
    expect(
  ...
  it('left-anchored: cursor at END, each highlight grows the left edge toward start', () => {
    ...
    const action = run(
      ...
      },
      {
        ...
        docReader: reader({ locate: () => [{ node: 't', start: 0, end: 10 }] }),
      }
    );
  ...
  it('right-anchored with a non-zero start offsets grow from start', () => {
    ...
    const action = run(
      ...
      },
      {
        ...
        docReader: reader({ locate: () => [{ node: 't', start: 4, end: 11 }] }),
      }
    );
    const hl = highlights(action.steps);
    expect(hl.map((h) => [h.x.span.start, h.x.span.end])).toEqual([
  ...
  it('sweeps=0 → exactly one final highlight, no incremental sweeps', () => {
    const action = run(
      ...
      },
      {
        ...
        docReader: reader({ locate: () => [{ node: 't', start: 2, end: 8 }] }),
      }
    );
    const hl = highlights(action.steps);
  ...
  it('sweeps=5 → six highlights with monotonically growing spans (left)', () => {
    ...
    const action = run(
      ...
      },
      {
        ...
        docReader: reader({ locate: () => [{ node: 't', start: 0, end: 12 }] }),
      }
    );
    const hl = highlights(action.steps);
  ...
  it('len=0 (degenerate, start===end): all sweeps collapse to a zero-width span', () => {
    const action = run(
      ...
      },
      {
        ...
        docReader: reader({ locate: () => [{ node: 't', start: 5, end: 5 }] }),
      }
    );
    const hl = highlights(action.steps);
    expect(hl).toHaveLength(6);
    expect(hl.every((h) => h.x.span.start === 5 && h.x.span.end === 5)).toBe(
      true
    );
...
describe('sweepSelect — pause ms come straight from the integer draws', () => {
  it('0 sweeps → only the settle pause', () => {
    const action = run(
      ...
      {
        randomSource: mockRandomSource({ integer: [30, 0, 90] }),
        docReader: reader({ locate: () => [{ node: 't', start: 0, end: 4 }] }),
      }
    );
  ...
  it('5 sweeps → 5 sweep pauses then the settle pause', () => {
    const action = run(
      ...
      {
        randomSource: mockRandomSource({
          integer: [30, 5, 109, 109, 109, 109, 109, 199],
        }),
        docReader: reader({ locate: () => [{ node: 't', start: 0, end: 4 }] }),
      }
    );
...
describe('retype (setText) — delete branch and ordering', () => {
  it('non-empty target: selectAll → preDelete pause → removeText → cursor 0 → typeText', () => {
    ...
    expect(action.steps).toEqual<DocumentOpStep[]>([
      ...
      {
        kind: 'awareness',
        x: { type: 'highlight', node: 'b', span: { start: 0, end: 3 } },
      },
