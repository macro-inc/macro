...
- The XML shows the document's typed AST: block nodes (`<p>`, `<h1>`–`<h6>`, `<blockquote>`, `<custom-code>`, `<ul>`, `<li>`, `<table>`, etc.) each with an `id` attribute; inline text runs as `<t id="...">` with format attributes (`bold="true"`, `italic="true"`, `underline="true"`, `strikethrough="true"`, `code="true"`).
