...
function formatToolCall(call: { toolName: string; input: unknown }, output: unknown): string {
	const lines: string[] = [];
...
function formatStep(step: StepResult<ToolSet>, i: number): string {
	...
	for (let j = 0; j < step.toolCalls.length; j++) {
		...
		lines.push("");
		lines.push(formatToolCall(
			call as unknown as { toolName: string; input: unknown },
			result != null ? (result as unknown as { output: unknown }).output : undefined,
		));
	}

	const { inputTokens, outputTokens } = step.usage;
	lines.push("");
	lines.push(`*${inputTokens.toLocaleString()} in / ${outputTokens.toLocaleString()} out*`);

	return lines.join("\n");
}

function formatTrace(meta: TraceMeta, steps: StepResult<ToolSet>[], usage: Usage): string {
	...
	if (meta.initialDocument) {
		sections.push("", "**document before:**", "```xml", meta.initialDocument, "```");
	}
