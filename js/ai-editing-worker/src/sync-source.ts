...
export class WorkerSyncSource implements LiveSyncSource {
	...
	private onMessage(ev: MessageEvent) {
		...
		if (msg.isRemoteInitialSync()) {
			...
			this.emit({ type: "update", update: (msg.value as { update: Uint8Array }).update });
...
/**
 * Minimal {@link Awareness} the engine requires but the worker doesn't really
 * use — the AI's live cursors are broadcast separately via `realAwarenessSource`
 * → `pushAwareness`. Backed by an EphemeralStore so `getEncodedLocalAwareness`
 * still returns a valid (empty) blob on engine stop.
 */
