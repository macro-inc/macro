 ...
 * awaits until one is returned — a true counting semaphore whose count is the
 * pool size. Parallel writer queues share one pool instance.
 *
...
export class LoroPeerPool {
   ...
   *  edits round-robin across distinct peers, while contended borrows still
   *  block on capacity like a counting semaphore. */
   ...
   *   of duplicates. Tests pass an explicit list for determinism.
   */
  ...
  static fromSeed(base: bigint, size: number): LoroPeerPool {
  ...
  async with<T>(fn: (id: bigint) => T | Promise<T>): Promise<T> {
