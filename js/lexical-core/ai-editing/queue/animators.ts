...
/** Type `text` into a node one char at a time from `from`, cursor following. A
 *  lead-in beat separates positioning (caret placement) from typing. */
function typeText(node: NodeRef, text: string, from: Offset, ctx: AnimatorCtx): DocumentOpStep[] {
  ...
  for (let k = 0; k < text.length; k++) {
    steps.push(edit({ fn: 'insertText', node, at: from + k, text: text[k]! }));
    steps.push(cursor(node, from + k + 1));
    steps.push({ kind: 'pause', ms: Math.round(msPerChar * randomSource.real(ranges.typeJitter)) });
  }
  return steps;
}
