...
import { applyEdit, describe as describeOp, runQueue, summarize, type OpResult } from './executor';
...
function recordingWriter(throwOn?: { fn: string; error: string } | Array<{ fn: string; error: string }>) {
  const throwers = throwOn ? (Array.isArray(throwOn) ? throwOn : [throwOn]) : [];
...
async function run(ops: DocumentOp[], writer: DocWriter, opts: { docReader?: DocReader; sleep?: (ms: number) => Promise<void> } = {}) {
  ...
  const results = await runQueue({
    queue: DocumentOpQueue.from(ops, DEFAULT_QUEUE_PARAMS),
...
describe('runQueue -- happy path', () => {
  ...
  it('awaits pauses through the injected sleep', async () => {
    ...
    await runQueue({
      queue: DocumentOpQueue.from([{ kind: 'setBlockType', id: 'b1', block: 'quote' }], DEFAULT_QUEUE_PARAMS),
      randomSource: mockRandomSource(),
      docReader: reader(),
...
describe('runQueue -- awareness ordering & sleep', () => {
  ...
  it('every pause ms is passed to the injected sleep, in order', async () => {
    ...
    await runQueue({
      queue: DocumentOpQueue.from([{ kind: 'setChecked', id: 'b1', checked: true }], DEFAULT_QUEUE_PARAMS),
  ...
  it('edits and awareness interleave: a removeBlock pumps highlights before the remove', async () => {
    ...
    await runQueue({
      queue: DocumentOpQueue.from([{ kind: 'removeBlock', id: 'b1' }], DEFAULT_QUEUE_PARAMS),
...
describe('summarize', () => {
  it('empty results → "no operations"', () => {
    expect(summarize([])).toBe('no operations');
  });
  it('a failed result includes the op kind prefix', () => {
    ...
    expect(summarize(results)).toBe('✗ removeBlock: planning blew up');
  });
  it('mixes ok and failed lines', () => {
    const results: OpResult[] = [
      { ok: true, op: { kind: 'removeBlock', id: 'b1' }, summary: 'removed {b1}' },
      { ok: false, op: { kind: 'setText', id: 'b2', text: 'x' }, error: 'nope' },
    ];
    expect(summarize(results)).toBe('✓ removed {b1}\n✗ setText: nope');
  });
});
...
describe('runQueue -- empty queue short-circuit', () => {
  it('never touches the writer, reader, or awareness', async () => {
    ...
    const { w, calls } = recordingWriter();
    const results = await runQueue({
      queue: DocumentOpQueue.from([], DEFAULT_QUEUE_PARAMS),
      randomSource: mockRandomSource(),
      docReader: reader(),
      docWriter: w,
      awarenessSource: { apply },
