...
import { DEFAULT_QUEUE_PARAMS, DocumentOpQueue } from './document-op-queue';
...
/** Run a single op through the queue and return its action's steps. */
function run(op: DocumentOp, opts: { randomSource?: RandomSource; docReader?: DocReader } = {}) {
  const q = DocumentOpQueue.from([op], { speed: 400, ranges: DEFAULT_RANGES }); // pin 400 → msPerChar 30 for exact-ms assertions
  return q.step({ randomSource: opts.randomSource ?? mockRandomSource(), docReader: opts.docReader ?? reader() });
}
...
describe('setText animator — select-all, delete, type (full flow)', () => {
  it('selects the existing text, deletes it, then types char by char', () => {
    // integer draws: preSelect(30), sweepCount=3, 3 sweep pauses(75), settle(145), preDelete(190), typeText lead-in settle(150).
    // real draws: typeJitter 1.05 per char → round(30*1.05)=32.
    const action = run(
    ...
    expect(action.steps).toEqual<DocumentOpStep[]>([
      ...
      { kind: 'edit', y: { fn: 'insertText', node: 'b1', at: 0, text: 'H' } },
      { kind: 'awareness', x: { type: 'cursor', node: 'b1', at: 1 } },
      { kind: 'pause', ms: 32 },
      { kind: 'edit', y: { fn: 'insertText', node: 'b1', at: 1, text: 'i' } },
...
describe('append / prepend animators', () => {
  ...
  it('prependText types from offset 0', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      ...
      { fn: 'insertText', node: 'b1', at: 1, text: 'B' },
    ]);
  });
});
...
describe('insertBlock animator', () => {
  it('inserts an EMPTY typed block, then types its text into the ref', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      ...
      { fn: 'insertText', node: 'ref-1', at: 0, text: 'H' },
      { fn: 'insertText', node: 'ref-1', at: 1, text: 'i' },
    ]);
  });
  ...
  it('divider: drafts the dashes, then swaps them for the rule', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      ...
      { fn: 'insertText', node: 'ref-3~draft', at: 1, text: '-' },
      { fn: 'insertText', node: 'ref-3~draft', at: 2, text: '-' },
      { fn: 'removeNode', node: 'ref-3~draft' },
      { fn: 'insertNode', ref: 'ref-3', spec: { block: 'divider' }, at: { after: 'b1' } },
    ]);
    // the caret walks across the three dashes (offsets 1,2,3)
...
describe('default speed', () => {
  it('is 2x (800 wpm → ~15ms/char) at DEFAULT_QUEUE_PARAMS speed', () => {
    const q = DocumentOpQueue.from([{ kind: 'appendText', id: 'b1', text: 'x' }], DEFAULT_QUEUE_PARAMS);
    const action = q.step({ randomSource: mockRandomSource({ real: 1.05 }), docReader: reader({ textLength: () => 0 }) });
    // first pause is the typeText lead-in (integer 0); the per-char pause is the last.
    const pause = action.steps.filter((s) => s.kind === 'pause').at(-1);
...
describe('setCell animator', () => {
  it('resolves the cell node and retypes it', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      ...
      { fn: 'insertText', node: 'cellX', at: 0, text: 'H' },
      { fn: 'insertText', node: 'cellX', at: 1, text: 'i' },
    ]);
  });
  ...
  it('filling a freshly-built (empty) cell just types — cursor walks, no delete', () => {
    ...
    // the cursor walks with each char (offsets 1 then 2)
    const cursors = action.steps.filter((s) => s.kind === 'awareness' && (s as any).x.type === 'cursor').map((s) => (s as any).x.at);
...
describe('queue stepping & done', () => {
  it('reports done only on the last op and yields empty after exhaustion', () => {
    const q = DocumentOpQueue.from([
      { kind: 'removeBlock', id: 'b1' },
      { kind: 'removeBlock', id: 'b2' },
    ], DEFAULT_QUEUE_PARAMS);
    const args = { randomSource: mockRandomSource(), docReader: reader() };
    expect(q.step(args).done).toBe(false);
    expect(q.remaining).toBe(1);
    expect(q.step(args).done).toBe(true);
    expect(q.step(args)).toEqual({ done: true, steps: [] });
  });
});
...
describe('typeText — per-char pause = round(msPerChar * typeJitter draw)', () => {
  ...
  it('multi-char offset progression: insert/cursor/pause per char from the start offset', () => {
    // prependText types from offset 0
    ...
    expect(action.steps).toEqual<DocumentOpStep[]>([
      ...
      { kind: 'pause', ms: 0 }, // typeText lead-in (default integer 0)
      { kind: 'edit', y: { fn: 'insertText', node: 'b', at: 0, text: 'a' } },
      { kind: 'awareness', x: { type: 'cursor', node: 'b', at: 1 } },
      { kind: 'pause', ms: 32 },
      { kind: 'edit', y: { fn: 'insertText', node: 'b', at: 1, text: 'b' } },
      { kind: 'awareness', x: { type: 'cursor', node: 'b', at: 2 } },
      { kind: 'pause', ms: 32 },
      { kind: 'edit', y: { fn: 'insertText', node: 'b', at: 2, text: 'c' } },
  ...
  it('appendText offsets start at the existing length', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      { fn: 'insertText', node: 'b', at: 7, text: 'X' },
      { fn: 'insertText', node: 'b', at: 8, text: 'Y' },
    ]);
    expect(cursors(action.steps).map((c) => c.x.at)).toEqual([7, 8, 9]); // initial + after each char
  });
...
describe('retype (setText) — delete branch and ordering', () => {
  ...
  it('empty target (textLength 0): no preDelete pause, no removeText, no cursor-0 reset', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      { fn: 'insertText', node: 'b', at: 0, text: 'H' },
      { fn: 'insertText', node: 'b', at: 1, text: 'i' },
    ]);
    ...
    // no preDelete pause — preSelect(30) + settle(90) + typeText lead-in(150) + 2 type pauses(18)
    expect(pauses(action.steps)).toEqual([30, 90, 150, 18, 18]);
  });
...
describe('insertBlock animator — typed vs whole', () => {
  it('heading is a TYPED_BLOCK: inserted empty then typed', () => {
    ...
    expect(onlyEdits(action.steps)).toEqual([
      ...
      { fn: 'insertText', node: 'r', at: 0, text: 'H' },
      { fn: 'insertText', node: 'r', at: 1, text: 'i' },
    ]);
  });
