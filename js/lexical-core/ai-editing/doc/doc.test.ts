...
import { applyEdit } from '../queue/executor';
...
describe('Doc — inline formatting (reuses ai-toolkit)', () => {
  ...
  it('markText on highlights (wraps in a mark node); off throws a clean EditError', () => {
    const { s, ids } = setup('warn here');
    expect(() => new Doc(s).markText(ids[0]!, 'warn', false, {})).toThrow(/todo/);
  ...
  it('linkText wraps; unlink (null) throws for now', () => {
    const { s, ids } = setup('see docs');
    expect(() => new Doc(s).linkText(ids[0]!, 'docs', null, {})).toThrow(/todo/);
...
describe('buildNode — each spec builds the right node type', () => {
  const elementBlocks: Array<[string, any, string]> = [
    ...
    ['code', { block: 'code', language: 'ts', text: 'c' }, 'code'],
  ...
  it('mention spec throws "todo"', () => {
    const { s } = setup('x');
    expect(() =>
      edit(s, () => buildNode({ inline: 'mention', mention: { kind: 'group', groupAlias: 'g' } }))
    ).toThrow('todo');
  });
});
