...
import { $getId, $updateAllNodeIds } from '../../plugins/nodeIdPlugin';
import {
  $appendText,
  $clearFormat,
  $formatTextInBlock,
...
import { $appendBlock, $mergeBlocks, $moveBlock, $prependBlock, $setText, $splitBlock } from '../ai-toolkit/blocks';
...
export class Doc implements DocReader, DocWriter {
  ...
  public markText(node: NodeRef, match: string, on: boolean, scope: Scope): void {
    if (!on) throw new EditError('todo: removing a highlight is not supported yet');
    this.tx(() => $highlightInBlock(this.block(node), match, scope));
  }

  public linkText(node: NodeRef, match: string, url: string | null, scope: Scope): void {
    if (url === null) throw new EditError('todo: removing a link is not supported yet');
    this.tx(() => $wrapInLink(this.block(node), match, url, scope));
  }
  ...
  public setBlockType(node: NodeRef, block: 'paragraph' | 'heading' | 'quote' | 'code', opts: { level?: number; language?: string }): void {
    this.tx(() => $modifyNode(this.s, this.id(node), { op: 'blockType', block, level: opts.level }));
  }
  ...
  public moveNode(node: NodeRef, at: Position): void {
    this.tx(() => {
      ...
      if ('after' in at) $moveBlock(block, { afterId: this.id(at.after) }, this.s);
        ...
        block.remove();
        if ('appendToRoot' in at) $appendBlock(this.s, block);
        else $prependBlock(this.s, block);
      }
    });
  }
...
/** Turn a declarative spec into real Lexical nodes. Mentions are stubbed. */
export function buildNode(spec: NodeSpec): LexicalNode {
  return match(spec)
    ...
    .with({ block: 'code' }, (s) => withText($createCodeNode(), s.text))
    .with({ block: 'list' }, (s) => {
