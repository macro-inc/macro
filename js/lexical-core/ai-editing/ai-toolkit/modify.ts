...
import { $blockNode, $setBlockType, $setText, type BlockType } from './blocks';
...
export type NodeChange =
  | { op: 'blockType'; block: BlockType; level?: number }
  | { op: 'text'; text: string }
...
export function $modifyNode(
  ...
  return match(change)
    ...
    .with({ op: 'blockType' }, (c) =>
      $setBlockType(s, $asBlock(node, label), () => $blockNode(c.block, { level: c.level }))
    )
    .with({ op: 'text' }, (c) => {
