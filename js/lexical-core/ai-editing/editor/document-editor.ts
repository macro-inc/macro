 ...
 * running the AI's snippet just fills `ops`, which the queue then animates and
 * applies. A method that references an unknown id (or violates a sanity check)
 * throws `EditError` on the spot, so the tool can report it to the model before
 * anything is applied.
 *
...
export class DocumentEditor {
  ...
  // ── inline formatting (by block id + substring) ──────────────────────────
  ...
  // ── inline formatting (by text-node id directly) ─────────────────────────
  ...
  makeParagraph(id: NodeId): this { return this.setBlockType(id, 'paragraph'); }
  makeHeading(id: NodeId, level: number): this {
    if (level < 1 || level > 6) throw new EditError(`heading level must be 1-6, got ${level}`);
    return this.setBlockType(id, 'heading', { level });
  }
  makeQuote(id: NodeId): this { return this.setBlockType(id, 'quote'); }
  makeCodeBlock(id: NodeId, language?: string): this { return this.setBlockType(id, 'code', { language }); }
  ...
  // ── structure (creators return a Ref) ────────────────────────────────────
  ...
  // ── tables ─────────────────────────────────────────────────────────────
  // A table is built the way a person makes one: the empty grid appears, then
  // each cell is filled in turn. So we insert an empty table of the right shape
  // and emit a `setCell` per non-empty cell — each animates as cursor-to-cell +
  // typing, so the table fills in left-to-right, top-to-bottom.
  ...
  // ── media / math / structural inline (creators return a Ref) ─────────────
  ...
  // ── mentions (stubbed; need real entity ids) ─────────────────────────────
  mentionUser(_blockId: NodeId, _at: number, _entity: { userId: string; email: string }): Ref {
    throw new Error('todo');
  }
  mentionContact(_blockId: NodeId, _at: number, _entity: { contactId: string; name: string; emailOrDomain: string; isCompany: boolean }): Ref {
    throw new Error('todo');
  }
  mentionGroup(_blockId: NodeId, _at: number, _entity: { groupAlias: string }): Ref {
    throw new Error('todo');
  }
  mentionDocument(_blockId: NodeId, _at: number, _entity: { documentId: string; documentName: string; blockName: string }): Ref {
    throw new Error('todo');
  }
}
