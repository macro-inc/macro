...
/// Deletes every transcript-segment doc for a given call id. Each doc's
/// `_id` is `{call_id}:{transcript_id}`, so we fan out via a term match on
/// `entity_id` (= call_id).
...
/// Deletes every call-record segment for a given channel.
#[tracing::instrument(skip(client))]
