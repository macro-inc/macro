...
pub fn should_wildcard_field_query_builder(field: &str, values: &[String]) -> QueryType {
    let mut should_query = BoolQueryBuilder::new().minimum_should_match(1);
    let wildcard_queries: Vec<WildcardQuery> = values
        .iter()
        .map(|value| WildcardQuery::new(field, value).case_insensitive(true))
        .collect();
    for query in wildcard_queries {
        should_query = should_query.should(QueryType::WildCard(query));
    }
