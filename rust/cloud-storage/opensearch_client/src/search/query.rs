...
/// Generates the term queries SearchOn::NameContent for unified search
pub(crate) fn generate_name_content_unified_query(
    title_keys: &[&str],
    terms: &[String],
) -> QueryType {
    let mut terms_must_query = BoolQueryBuilder::new();
    ...
    let queries: Vec<QueryType> = terms
        ...
        .map(|term| {
            ...
            for title_key in title_keys {
                bool_query.should(QueryType::MatchPhrasePrefix(
                    MatchPhrasePrefixQuery::new(title_key, term).boost(1000.0),
                ));

                bool_query.should(QueryType::Match(
                    MatchQuery::new(title_key, term)
                        .boost(0.1)
            ...
            bool_query.should(QueryType::MatchPhrasePrefix(
                MatchPhrasePrefixQuery::new("content", term).boost(900.0),
            ));

            bool_query.should(QueryType::Match(
                MatchQuery::new("content", term)
                    .boost(0.09)
                    .minimum_should_match(&term.split(' ').count().to_string()), // TODO: we may need to play around with this to get the best highlight match
            ));
