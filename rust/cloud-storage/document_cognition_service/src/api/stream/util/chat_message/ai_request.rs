...
use attachment::{
    AttachmentContent, AttachmentPart, AttachmentReference, Attachments, image::ImageData,
};
...
fn resolved_parts_to_attachments(parts: Vec<ResolvedMessagePart>) -> Option<Attachments> {
...
fn resolved_part_to_content(part: ResolvedMessagePart) -> AttachmentContent {
    match part {
        ResolvedMessagePart::Attachment { name, parts } => {
            let attachment_parts: Vec<AttachmentPart> = parts
                .into_iter()
                .map(resolved_part_to_attachment_part)
                .collect();

            AttachmentContent {
                reference: AttachmentReference::DssFile {
                    id: String::new(),
                },
                name: Some(name),
        ...
        ResolvedMessagePart::Image(img) => AttachmentContent {
            reference: AttachmentReference::SfsImage {
                url: String::new(),
            },
            name: None,
...
fn resolved_part_to_attachment_part(part: ResolvedMessagePart) -> AttachmentPart {
    match part {
