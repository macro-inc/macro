...
mod tests {
    ...
    async fn test_get_properties_by_team(pool: Pool<Postgres>) -> anyhow::Result<()> {
        const _: &sqlx::migrate::Migrator = &MACRO_DB_MIGRATIONS; // Dummy reference for IDE

        let properties = get_properties(&pool, Some(team_1()), None, false).await?;
    ...
    async fn test_get_properties_with_options(pool: Pool<Postgres>) -> anyhow::Result<()> {
        ...
        let properties = get_properties_with_options(&pool, Some(team_1()), None, false).await?;
