...
use models_email::service::message::{MessageWithBodyReplyless, ParsedMessage};
use sqlx::PgPool;
use sqlx::types::Uuid;
...
pub async fn get_parsed_message_by_id(
    pool: &PgPool,
    message_id: &Uuid,
) -> anyhow::Result<Option<message::ParsedMessage>> {
    ...
    Ok(Some(ParsedMessage::from(MessageWithBodyReplyless::from(
        service_message,
    ))))
}
    ...
    let tasks: Vec<_> = db_messages
        ...
        .map(|message| {
            ...
            async move {
                ...
                ParsedMessage::from(MessageWithBodyReplyless::from(service_message))
            }
        })
        .collect();
