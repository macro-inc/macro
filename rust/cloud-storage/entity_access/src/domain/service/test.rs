...
async fn test_get_document_access_returns_level_from_repo() {
    ...
    let result = service
        .get_access_level("user-1", "doc-1", EntityType::Document)
        .await;
...
async fn test_get_chat_access_returns_level_from_repo() {
    ...
    let result = service
        .get_access_level("user-1", "chat-1", EntityType::Chat)
        .await;
...
async fn test_get_project_access_returns_level_from_repo() {
    ...
    let result = service
        .get_access_level("user-1", "proj-1", EntityType::Project)
        .await;
...
async fn test_get_thread_access_returns_level_from_repo() {
    ...
    let result = service
        .get_access_level("user-1", "thread-1", EntityType::Thread)
        .await;
...
async fn test_get_channel_access_with_invalid_uuid_returns_error() {
    ...
    let result = service
        .get_access_level("user-1", "not-a-uuid", EntityType::Channel)
        .await;
...
async fn test_check_access_sufficient_level_returns_actual_level() {
    ...
    let result = service
        .check_access("user-1", "doc-1", EntityType::Document, AccessLevel::View)
        .await;
...
async fn test_check_access_insufficient_level_returns_unauthorized() {
    ...
    let result = service
        .check_access("user-1", "doc-1", EntityType::Document, AccessLevel::Edit)
        .await;
...
async fn test_unsupported_entity_type_returns_none() {
    ...
    // Company, Task, and User entity types are not supported
    let result = service
        .get_access_level("user-1", "company-1", EntityType::Company)
        .await;
    assert_eq!(result.unwrap(), None);

    let result = service
        .get_access_level("user-1", "task-1", EntityType::Task)
        .await;
    assert_eq!(result.unwrap(), None);

    let result = service
        .get_access_level("user-1", "user-1", EntityType::User)
        .await;
    assert_eq!(result.unwrap(), None);
}
