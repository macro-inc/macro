...
pub async fn get_location_handler_v3(
    ...
    let make_result = |response: anyhow::Result<LocationResponseV3>| {
        ...
        let max_age = state.config.document_storage_service_presigned_url_browser_cache_expiry_seconds;
...
async fn get_editable_url(
    ...
    let signed_options = get_cloudfront_signed_options(
        &state.config.vars.document_storage_service_cloudfront_signer_public_key_id,
        state.config.document_storage_service_cloudfront_signer_private_key.as_ref(),
        state.config.document_storage_service_presigned_url_expiry_seconds,
    );

    let signed_url = get_presigned_url(
        &state.config.vars.document_storage_service_cloudfront_distribution_url,
        &document_key,
        &signed_options,
    )?;
...
async fn get_docx_urls(
    ...
    let cloudfront_distribution_url = state.config.vars.document_storage_service_cloudfront_distribution_url.as_ref();
