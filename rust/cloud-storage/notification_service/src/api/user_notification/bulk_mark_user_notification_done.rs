...
#[utoipa::path(
        patch,
        operation_id = "bulk_mark_user_notification_done",
        path = "/user_notifications/bulk/done",
        responses(
...
pub async fn handler(
    ...
    tracing::info!("bulk_mark_user_notification_done");
    ...
    .await
    .map_err(|e| {
        tracing::error!(error=?e, "failed to patch user notifications done");
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                message: "failed to patch user notifications done",
            }),
        )
            .into_response()
    })?;
