...
/// Marks the user's notification as done for a given event item id and type.
#[utoipa::path(
        patch,
        operation_id = "bulk_mark_user_notification_done_by_event",
        path = "/user_notifications/item/{event_item_id}/done",
        params(
...
pub async fn handler(
    ...
    tracing::info!("bulk_mark_user_notification_done_by_event");

    let notification_ids =
        notification_db_client::user_notification::patch::done::bulk_patch_done_by_event(
            &ctx.db,
        ...
        .await
        .map_err(|e| {
            tracing::error!(error=?e, "failed to patch user notification done for event");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    message: "failed to patch user notification done for event",
                }),
            )
                .into_response()
        })?;

    if let Some(first_uuid) = notification_ids.first() {
        // We only need the first notification as all basic info will be the same
        let basic_notification_information =
            notification_db_client::notification::get::get_basic_notification(&ctx.db, first_uuid)
                .await
                .map_err(|e| {
        ...
        tokio::spawn({
            notification::send::push::remove::clear_push_notifications_basic(
                ctx,
                Cow::Owned(user_context.user_id.clone()),
