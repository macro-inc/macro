...
pub(crate) async fn process_message(
        ...
        .context("unable to rate limit notification")?
    {
        tracing::info!(
            ...
            sender_id=?sender_id,
            event_item=?notification.notification_entity,
    ...
    let user_ids = filter_user_ids(
        &ctx.db,
        &notification.notification_entity.entity_id,
        user_ids,
    )
    .await?;

    let macro_user_ids: Vec<_> = user_ids
        .iter()
        .filter_map(|id| {
            Some(
                MacroUserIdStr::parse_from_str(id.as_ref())
                    .ok()?
                    .into_owned(),
            )
        })
    ...
    if notification.notification_event.event_type() == NotificationEventType::RejectTeamInvite {
        notification_db_client::notification::delete::delete_notification_by_event_item(
            &ctx.db,
            &notification.notification_entity.entity_id,
            &notification.notification_entity.entity_type.to_string(),
        )
        .await?;
    }
    ...
    let notification = create_notification(&ctx.db, notification, &user_ids).await?;
    ...
    let notifications_with_user_data = populate_user_data(notification, &macro_user_ids);

    let list_user_notifications: Vec<_> = notifications_with_user_data
        .values()
        .flatten()
        .cloned()
        .collect();
    ...
    let users_sent_connection_gateway = send::connection_gateway::send_connection_gateway(
        ...
        list_user_notifications.as_slice(),
    )
    .await
    ...
    let users_to_push = macro_user_ids
        .iter()
    ...
    #[cfg(feature = "send_email_notifications")]
    {
        ...
        let email_results: Vec<anyhow::Result<()>> =
            futures::stream::iter(notifications_with_user_data.values().flatten())
                .then(|notification| {
                    let ctx_clone = ctx.clone();
                    async move {
                        send::email::process_email_notifications(&ctx_clone, notification)
                            .await
