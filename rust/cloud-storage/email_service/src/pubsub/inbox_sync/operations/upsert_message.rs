...
async fn fetch_and_insert_thread(
    ...
    // process threads
    process_threads_pre_insert(&ctx.db, &ctx.sfs_client, &mut threads, ctx.environment).await;
...
async fn process_and_insert_message(
    ...
    process_message_pre_insert(&ctx.db, &ctx.sfs_client, &mut message, ctx.environment).await;
