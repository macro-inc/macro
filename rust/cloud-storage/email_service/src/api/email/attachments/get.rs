...
async fn get_presigned_url(state: &ApiContext, key: &str) -> anyhow::Result<String> {
    ...
    let public_key_id = state.config.email_service_cloudfront_signer_public_key_id.clone();
    let private_key = state.config.email_service_cloudfront_signer_private_key.as_ref();
    let url = state.config.email_service_cloudfront_distribution_url.clone();
