...
impl SearchServiceClient {
    pub async fn search_documents(
        &self,
        user_id: &str,
        body: models_search::document::DocumentSearchRequest,
        page: i64,
        page_size: i64,
    ) -> anyhow::Result<models_search::document::DocumentSearchResponse> {
        let res = self
            .client
            .post(format!(
                "{}/internal/search/document?page={}&page_size={}",
        ...
        match res.status() {
            reqwest::StatusCode::OK => {
                let result = res
                    .json::<models_search::document::DocumentSearchResponse>()
                    .await?;
                Ok(result)
            }
            status_code => {
