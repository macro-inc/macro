...
impl SearchServiceClient {
    pub async fn search_chats(
        &self,
        user_id: &str,
        body: models_search::chat::ChatSearchRequest,
        page: i64,
        page_size: i64,
    ) -> anyhow::Result<models_search::chat::ChatSearchResponse> {
        let res = self
            .client
            .post(format!(
                "{}/internal/search/chat?page={}&page_size={}",
        ...
        match res.status() {
            reqwest::StatusCode::OK => {
                let result = res
                    .json::<models_search::chat::ChatSearchResponse>()
                    .await?;
                Ok(result)
            }
            status_code => {
