...
```sh
cargo run -p graphql_soup --bin graphql_soup_schema -- schema.graphql   # from rust/cloud-storage
```

The exported `schema.graphql` feeds both the client codegen
(`js/app && bun gen-graphql`) and the cache metadata codegen (hashing it —
