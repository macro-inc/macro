...
pub(in crate::api) mod get_theme_preferences;
...
pub(in crate::api) mod patch_theme_preferences;
...
fn router_with_auth(state: ApiContext, jwt_args: JwtValidationArgs) -> Router<ApiContext> {
    Router::new()
        ...
        .route(
            "/theme_preferences",
            get(get_theme_preferences::handler).patch(patch_theme_preferences::handler),
        )
        .route(
            "/quota",
