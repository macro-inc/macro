...
"#,
        ...
        .try_map(|r| match r.item_type.as_ref() {
            ...
            "project" => Ok(SoupItem::Project(SoupProject { id: Uuid::parse_str(&r.id).map_err(type_err)?, name: r.name, owner_id: MacroUserIdStr::parse_from_str(&r.user_id).map_err(type_err)?.into_owned(), parent_id: r.project_id.as_deref().map(Uuid::parse_str).transpose().map_err(type_err)?, created_at: r.created_at, updated_at: r.updated_at, viewed_at: r.viewed_at })),
