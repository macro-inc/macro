...
async fn test_search_email_subjects_ids_only_mode(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    assert_eq!(response.items[0].name, "Re: Monthly <macro_em>Invoice</macro_em> - December");

    assert_eq!(
        response.items[1].entity_id.to_string(),
        "11111111-1111-1111-1111-111111111111"
    );
    assert_eq!(response.items[1].name, "<macro_em>Invoice</macro_em> from Q1 2024");
...
async fn test_search_email_subjects_normal_mode_owned_threads(
    ...
    assert_eq!(response.items[0].name, "IMPORTANT: <macro_em>INVOICE</macro_em> DUE");

    assert_eq!(
        response.items[1].entity_id.to_string(),
        "22222222-2222-2222-2222-222222222222"
    );
    assert_eq!(response.items[1].name, "Re: Monthly <macro_em>Invoice</macro_em> - December");
...
async fn test_search_email_subjects_partial_match(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    assert_eq!(response.items[0].name, "Fwd: Client <macro_em>Meet</macro_em>ing Tomorrow");
...
async fn test_search_email_subjects_multiple_messages_per_thread(
    ...
    let thread_result = response
        ...
        .unwrap();
    assert_eq!(thread_result.name, "<macro_em>Invoice</macro_em> from Q1 2024");
