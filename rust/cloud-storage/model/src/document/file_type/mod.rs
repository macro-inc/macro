...
static SUFFIX_TRIE: Lazy<ReversedSuffixTrie> = Lazy::new(|| {
...
macro_rules! generate_file_types {
    ($(($variant:ident, $str_name:expr, $mime_type:expr, $app_path:expr)),* $(,)?) => {
        ...
        ///
        #[derive(serde::Serialize, serde::Deserialize, Eq, PartialEq, Debug, Copy, Clone, ToSchema)]
        ...
        impl FileType {
            ...
            #[expect(clippy::should_implement_trait, reason = "FIXME implement FromStr instead")]
            pub fn from_str(file_type: &str) -> Option<Self> {
                ...
                match cleaned.as_str() {
                    $(
                        $str_name => Some(FileType::$variant),
                    )*
                    _ => {
                        tracing::warn!("invalid file type {file_type} attempted to be made from_string");
                        None
                    }
                }
            }
        ...
        impl ContentType {
            #[expect(clippy::should_implement_trait, reason = "FIXME implement FromStr instead")]
            pub fn from_str(mime_type: &str) -> Option<Self> {
                ...
                match cleaned.as_str() {
                    $(
                        $mime_type => Some(ContentType::$variant),
                    )*
                    _ => {
                        tracing::warn!("invalid mime type {mime_type} attempted to be made from string");
                        None
                    }
                }
            }
...
// Uses the from_str function from the macro to convert a string to a FileType
impl TryInto<FileType> for &str {
    type Error = anyhow::Error;
    fn try_into(self) -> Result<FileType, Self::Error> {
        FileType::from_str(self).ok_or_else(|| {
            anyhow::anyhow!("invalid file type {self} attempted to be made from_string")
        })
    }
}

impl FileType {
    pub fn clean_document_name(document_name: String) -> String {
        match FileType::split_suffix_match(document_name.as_str()) {
            Some((document_name, _)) => document_name.to_string(),
            None => document_name,
        }
    }
    ...
    pub fn is_suffix_match(filename: &str) -> bool {
        SUFFIX_TRIE.string_has_suffix(filename)
    }

    // Returns the file prefix and the extension (no period)
    pub fn split_suffix_match(filename: &str) -> Option<(&str, &str)> {
...
mod tests {
    ...
    fn test_clean_document_name() {
        assert_eq!(
            FileType::clean_document_name("report.docx".to_string()),
            "report"
        );
        assert_eq!(
            FileType::clean_document_name("proposal.pdf".to_string()),
            "proposal"
        );
        assert_eq!(
            FileType::clean_document_name("testing.md".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.py".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.html".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.txt".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("notes.backup.pdf".to_string()),
            "notes.backup"
        );

        assert_eq!(
            FileType::clean_document_name("testing.random".to_string()),
            "testing.random"
        );
        assert_eq!(FileType::clean_document_name("note.".to_string()), "note.");
        assert_eq!(FileType::clean_document_name("".to_string()), "");
        assert_eq!(
            FileType::clean_document_name("testing".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.zip".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.tar.gz".to_string()),
            "testing"
        );
        assert_eq!(
            FileType::clean_document_name("testing.js.map".to_string()),
            "testing"
        );
    }
