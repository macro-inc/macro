...
async fn get_access_level(
    ...
    if access_level.is_none() && entity_ref.entity_type == EntityType::Thread {
        if let Ok(thread_id) = uuid::Uuid::parse_str(&entity_ref.entity_id) {
            if let Ok(Some(owner_id)) =
                email_db_client::threads::get::get_macro_id_from_thread_id(&context.db, thread_id)
                    .await
            {
                if owner_id == user_id {
