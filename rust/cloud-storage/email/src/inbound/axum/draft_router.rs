...
use crate::domain::{
    models::{EmailErr, Link},
    ports::EmailService,
};

use super::api_types::{CreateDraftRequest, CreateDraftResponse};

/// State for the draft router, containing only the email service.
pub struct EmailDraftRouterState<T> {
    /// The email service.
    pub service: Arc<T>,
}

impl<T> Clone for EmailDraftRouterState<T> {
    fn clone(&self) -> Self {
        Self {
...
pub fn draft_router<S, T>(state: EmailDraftRouterState<T>) -> Router<S>
where
    S: Send + Sync + 'static,
    T: EmailService,
{
...
pub async fn create_draft_handler<T: EmailService>(
    State(state): State<EmailDraftRouterState<T>>,
    ...
    let draft = state.service.create_draft(&link, input).await?;
