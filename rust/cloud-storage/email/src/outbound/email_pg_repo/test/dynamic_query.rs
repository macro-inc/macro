...
// ---------------------------------------------------------------------------
// Importance-filter pagination under-fill.
//
// The candidate LIMIT must count threads that survive the importance match,
// not raw recency. Before the fix the candidate subquery limited by recency
// and the importance filter was applied afterwards in the CROSS JOIN LATERAL,
// so a page of recent non-important threads under-filled the result while
// more important threads still remained — surfacing as the soup `/ast` email
// signal returning e.g. 8 items alongside a non-null cursor. The fixture lays
// the most-recent threads out as non-important and the important ones older,
// which is exactly the layout that triggered the bug.
// ---------------------------------------------------------------------------
...
async fn test_importance_inbox_pagination_fills_page_and_advances_cursor(
    ...
    // With limit=3 and 4 important threads, the first page must be a full page
    // of the 3 most-recent important threads; the recent non-important threads
    // must not eat into the limit.
    let query = Query::new(
        None,
...
async fn test_importance_inbox_underfilled_page_has_no_more(
    ...
    // 4 important threads exist on link 1, well below the limit. The page must
    // hold exactly those 4 (recent non-important threads excluded), and a
    // follow-up cursor page must be empty — i.e. `len < limit` genuinely means
    // "last page", so the caller emits a null cursor rather than the spurious
    // one the old post-LIMIT filtering produced.
    let query = Query::new(
        None,
        SimpleSortMethod::UpdatedAt,
        importance_exclude_filter(),
