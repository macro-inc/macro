...
async fn test_create_team(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    let result = team_repo.create_team(&user_id, "team1", &"sub_test".parse().unwrap()).await?;
...
async fn test_move_github_app_installation_to_team_moves_existing_user_rows(
    ...
    let team = team_repo.create_team(&user_id, "GitHub Owner Team", &"sub_test".parse().unwrap()).await?;
