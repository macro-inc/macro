...
struct KeyConfig {
    ...
    identity_witness: Option<String>,
}
...
fn main() {
    ...
    if let Some(witness) = &config.identity_witness {
        if !config.keys.contains_key(witness) {
            errors.push(format!(
                "identity_witness `{witness}` must be a keyed type in [keys]"
            ));
        }
    }
    ...
    match &config.identity_witness {
        Some(w) => writeln!(
            out,
            "pub static IDENTITY_WITNESS: Option<&str> = Some({w:?});"
        )
        .unwrap(),
        None => writeln!(out, "pub static IDENTITY_WITNESS: Option<&str> = None;").unwrap(),
    }
