...
import { MACRO_ORG_NAME, stack } from './shared';

const PULUMI_NETWORKING_STACK = 'macro-core-networking';
const networkingStack = new pulumi.StackReference('vpc-stack', {
  name: `${MACRO_ORG_NAME}/${PULUMI_NETWORKING_STACK}/${stack}`,
});

export function get_vpc() {
  return {
    vpcId: networkingStack.getOutput('vpcId').apply(v => v as string),
    privateSubnetIds: networkingStack
      .getOutput('privateSubnetIds')
      .apply(v => v as string[]),
    publicSubnetIds: networkingStack
      .getOutput('publicSubnetIds')
      .apply(v => v as string[]),
  };
}
