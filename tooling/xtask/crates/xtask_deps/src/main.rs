//! `cargo x deps [--check]` regenerates (or, with `--check`, verifies) the two
...
fn main() -> Result<()> {
    ...
    match args.iter().map(String::as_str).collect::<Vec<_>>()[..] {
        [] => run_deps(false),
        ["--check"] => run_deps(true),
        _ => bail!("usage: cargo x deps [--check]"),
    }
}
...
fn run_deps(check: bool) -> Result<()> {
    ...
    let graph: PackageGraph = if hakari_changed {
        build_graph(false)?
    } else {
        graph
    };
    ...
    if !drift.is_empty() {
        bail!(
            "generated dependency artifacts are stale:\n  - {}\nrun `just hakari` (or `cargo x deps`) from the repository root and commit the result",
            drift.join("\n  - ")
        );
    }
    Ok(())
