...
# Zip first, then smoke-test what comes *back out* of the zip. Appetize never
# sees the build directory — it installs the archive — so testing $APP directly
# would leave the round-trip itself unverified, which is precisely where a
# preview can differ from a build that looked fine in CI.
ditto -c -k --sequesterRsrc --keepParent "$APP" artifacts/macro-sim.zip
rm -rf artifacts/roundtrip
ditto -x -k artifacts/macro-sim.zip artifacts/roundtrip
ROUNDTRIP_APP="artifacts/roundtrip/$(basename "$APP")"
test -d "$ROUNDTRIP_APP" || { echo "zip round-trip produced no .app" >&2; exit 1; }
...
xcrun simctl install "$DEVICE" "$ROUNDTRIP_APP"
...
# The archive was built before the smoke test so the test could run against it;
# drop the unpacked copy so it does not bloat the artifact upload.
rm -rf artifacts/roundtrip
