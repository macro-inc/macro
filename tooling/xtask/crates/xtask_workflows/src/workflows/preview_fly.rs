    ...
    Step::new("Build artifacts (parallel lanes)")
        .run(indoc::indoc! {r#"
            ...
            for img in $(docker compose --project-directory . -p macro \
                -f target/nix/arion-compose.yaml config --images | sort -u); do
              if ! docker image inspect "$img" >/dev/null 2>&1; then
                echo "premirror: skipping $img (not loaded locally)"
                continue
              fi
    ...
    Step::new("Deploy to Fly")
        .run(indoc::indoc! {r#"
            ...
            for img in $images; do
              ...
              # must already be Nix-loaded; never docker-pull registry bases.
              docker image inspect "$img" >/dev/null 2>&1 || {
                echo "missing local Nix image $img" >&2
                exit 1
              }
              id=$(docker image inspect -f '{{.Id}}' "$img")
