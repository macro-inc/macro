...
impl LocalEnv {
    ...
    pub fn to_env(&self) -> BTreeMap<String, String> {
        ...
        env.extend(self.public_env());
        env
    }

    /// Explicit CLI public URLs win over dotenv/Doppler/process defaults. Only
    /// browser-facing values belong here; Docker clients retain internal URLs.
    pub fn public_env(&self) -> BTreeMap<String, String> {
        let Some(origin) = &self.public_origin else {
            return BTreeMap::new();
        };
        BTreeMap::from([
            ("ALLOWED_ORIGINS".into(), origin.clone()),
            ("OVERRIDE_APP_SERVICE_URL".into(), origin.clone()),
            ("BASE_URL".into(), format!("{origin}/auth")),
            ("FUSIONAUTH_PUBLIC_URL".into(), origin.clone()),
            (
                "FUSIONAUTH_OAUTH_REDIRECT_URI".into(),
                format!("{origin}/auth/oauth/redirect"),
            ),
            ("PIPEDREAM_ALLOWED_ORIGINS".into(), origin.clone()),
            ("MCP_PUBLIC_URL".into(), format!("{origin}/cognition")),
            // Local-only configuration, generated by xtask, never deployed.
            ("LOCAL_AWS_PUBLIC_URL".into(), format!("{origin}/s3")),
            ("LOCAL_PUBLIC_ORIGIN".into(), origin.clone()),
            (
                "STATIC_FILE_SERVICE_URL".into(),
            ...
            (
                ...
                format!("{origin}/s3/{}", resources::DOC_STORAGE_BUCKET),
            ),
            (
                "EMAIL_SERVICE_CLOUDFRONT_DISTRIBUTION_URL".into(),
                format!("{origin}/s3/macro-email-attachments"),
            ),
        ])
    }
