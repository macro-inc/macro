...
source "$ROOT_DIR/tooling/scripts/lib/stage-ui.sh"
stage_ui_init
...
run_builds() {
  stage_run "Building Rust services image" "${compose_base[@]}" build rust_services_image

  if [ "$do_build" != true ]; then
    return
  fi

  stage_run "Building app services" "${compose_base[@]}" build websocket_service sync_service lexical_service

  if [ "$build_processors" = true ]; then
    stage_run "Building search processor" "${compose_base[@]}" build search_processing_service
  fi
}

compose_command() {
  compose_cmd=("${compose_base[@]}" up)
