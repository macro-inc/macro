...
"$FF" -y -loglevel error -framerate 240 -i "$FRAMES/%05d.png" -i audio/mix.wav \
  -filter_complex "$VF,scale=out_color_matrix=bt709:out_range=tv[v]" -map "[v]" -map 1:a -r 60 -crf 14 "${ENC[@]}" macro-trailer.mp4
"$FF" -y -loglevel error -framerate 240 -i "$FRAMES/%05d.png" -i audio/mix.wav \
  -filter_complex "$VF,scale=1080:1080:flags=lanczos:out_color_matrix=bt709:out_range=tv[v]" -map "[v]" -map 1:a -r 60 -crf 16 "${ENC[@]}" macro-trailer-1080.mp4
echo "encoded macro-trailer.mp4 and macro-trailer-1080.mp4"
