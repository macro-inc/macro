...
// LOCAL FORK: gated behind `subprocess`. This is the only module that touches
// `async-process`, which reaches `errno` and fails to build for wasm32 - and
// spawning an agent subprocess is meaningless in a browser anyway.
#[cfg(feature = "subprocess")]
mod acp_agent;
#[cfg(feature = "subprocess")]
pub use acp_agent::{AcpAgent, LineDirection};

// LOCAL FORK: the stdio transport is the subprocess transport (it borrows
// `LineDirection` from acp_agent), so it rides the same feature.
#[cfg(feature = "subprocess")]
mod stdio;
#[cfg(feature = "subprocess")]
pub use stdio::Stdio;
