...
const EMAIL_LAMBDA_BASE_NAME = 'email_scheduled_handler';
const REPO_ROOT = '../../..';
const HANDLER_BASE = `${REPO_ROOT}/services/${EMAIL_LAMBDA_BASE_NAME}`;
const ZIP_LOCATION = `${REPO_ROOT}/target/lambda/${EMAIL_LAMBDA_BASE_NAME}/bootstrap.zip`;
...
export class EmailScheduledHandler extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    const sqsPolicy = new aws.iam.Policy(
      `${EMAIL_LAMBDA_BASE_NAME}-sqs-policy`,
      {
    ...
    this.role = new aws.iam.Role(
      `${EMAIL_LAMBDA_BASE_NAME}-role`,
      {
        name: `${EMAIL_LAMBDA_BASE_NAME}-role-${stack}`,
    ...
    const lambda = new Lambda<EnvVars>(
      `${EMAIL_LAMBDA_BASE_NAME}-lambda`,
      {
        baseName: EMAIL_LAMBDA_BASE_NAME,
    ...
    new aws.cloudwatch.EventTarget(
      `${EMAIL_LAMBDA_BASE_NAME}-minutely-target`,
      {
        rule: this.rule.name,
        arn: this.lambda.arn,
      },
  ...
  setupLambdaAlarms() {
    new aws.cloudwatch.MetricAlarm(
      `${EMAIL_LAMBDA_BASE_NAME}-error-alarm`,
      {
        name: `${EMAIL_LAMBDA_BASE_NAME}-error-count-${stack}`,
        metricName: 'Errors',
        ...
        alarmDescription: `Alarm when ${EMAIL_LAMBDA_BASE_NAME} lambda experiences errors.`,
        actionsEnabled: true,
