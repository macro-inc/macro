...
export const SERVICE_DOMAIN_NAME = `calendar-event-local-tunnel${
  stack === 'prod' ? '' : `-${stack}`
}.${BASE_DOMAIN}`;

type CreateCalendarEventLocalTunnelArgs = {
...
export class CalendarEventLocalTunnel extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    {
      ...
      tags,
    }: CreateCalendarEventLocalTunnelArgs,
    opts?: pulumi.ComponentResourceOptions
  ) {
    super('my:components:CalendarEventLocalTunnel', name, {}, opts);
    this.tags = tags;
    ...
    const image = new EcrImage(
      `${BASE_NAME}-ecr-image-${stack}`,
      {
        ...
        platform,
        buildArgs: {
          SERVICE_NAME: 'calendar_event_local_tunnel',
        },
        tags: this.tags,
      },
      { parent: this }
    ...
    // lb — long idle timeout: subscribers hold SSE connections open and the
    // service keep-alives every 15 seconds.
    ...
    // service — pinned to a single task with no autoscaling: subscriber
    // fan-out is an in-memory map, so a webhook landing on one task while
    // the matching subscriber is connected to another would drop deliveries.
            ...
            service: {
              ...
              logConfiguration: {
                ...
                options: {
                  ...
                  dd_service: 'calendar-event-local-tunnel',
                  dd_source: 'fargate',
                  dd_tags: `project:calendar-event-local-tunnel, env:${stack}`,
                  provider: 'ecs',
                },
              },
              portMappings: [
