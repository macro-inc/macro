...
export const SERVICE_DOMAIN_NAME = `agent-proxy${
  stack === 'prod' ? '' : `-${stack}`
}.${BASE_DOMAIN}`;
...
/**
 * The agent proxy service. Unlike every other service in this repo, this
 * component pins `desiredCount` to 1 and forces a stop-then-start deployment
 * (rather than the usual start-then-stop rolling deploy) with no autoscaling
 * at all: `SessionRegistry` and `SharedRuntimeConnections`
 * (crates/agent_proxy) hold WebSocket-connection and session-routing state in
 * process memory with no cross-instance sync, so a second task running even
 * momentarily would split agent-runtime traffic across two instances that
 * can't see each other's state.
 */
export class AgentProxyService extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    super('my:components:AgentProxyService', name, {}, opts);
    const {
      vpc,
      tags,
      platform,
    ...
    const image = new EcrImage(
      `${BASE_NAME}-ecr-image-${stack}`,
      {
        ...
        platform,
        buildArgs: {
          SERVICE_NAME: 'agent_proxy_service',
        },
        tags,
      },
      { parent: this }
    ...
    const { targetGroup, lb, listener } = serviceLoadBalancer(this, {
      ...
      isPrivate,
      tags,
      // The shared agent-runtime WebSocket connections on this port are
      // long-lived (an external coding agent's session can run far longer
      // than the ALB's 60s default), matching connection-gateway's reasoning
            ...
            service: {
              ...
              logConfiguration: {
                ...
                options: {
                  ...
                  dd_service: 'agent-proxy-service',
                  dd_source: 'fargate',
                  dd_tags: `project:agent-proxy-service, env:${stack}`,
                  provider: 'ecs',
                },
              },
              portMappings: [
