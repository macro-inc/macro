...
import { AgentProxyService, SERVICE_DOMAIN_NAME } from './agent_proxy_service';
...
const service = new AgentProxyService(`agent-proxy-service-${stack}`, {
  ...
  containerEnvVars: [
    ...
    // The ALB terminates TLS on 443; the container only ever sees plain HTTP
    // on its own port, so it can't infer this on its own.
    {
      name: 'RUNTIME_PUBLIC_URL',
      value: `wss://${SERVICE_DOMAIN_NAME}`,
    },
    // Datadog
    {
      name: 'DD_SERVICE',
