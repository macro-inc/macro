...
const BASE_NAME = 'delete-document-worker';
const BASE_PATH = '../../../../rust/cloud-storage';

export const SERVICE_DOMAIN_NAME = `delete-document-worker${
  stack === 'prod' ? '' : `-${stack}`
}.${BASE_DOMAIN}`;
  ...
  constructor(
      ...
      `${BASE_NAME}-queue-policy`,
      {
        ...
        policy: {
          ...
          Statement: [
            {
              Action: ['sqs:*'],
              Resource: [pulumi.interpolate`${deleteDocumentQueueArn}`],
              Effect: 'Allow',
            },
          ],
        },
    ...
    const image = new EcrImage(
      `${BASE_NAME}-ecr-image-${stack}`,
      {
        ...
        buildArgs: {
          SERVICE_NAME: 'delete_document_worker',
        },
        platform,
        tags: this.tags,
      },
