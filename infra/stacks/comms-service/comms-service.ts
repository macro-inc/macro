...
import { createFrecencyTablePolicy, serviceLoadBalancer } from '@resources';
...
const BASE_NAME = 'comms-service';
const BASE_PATH = '../../../rust/cloud-storage';

export const SERVICE_DOMAIN_NAME = `comms-service${
  stack === 'prod' ? '' : `-${stack}`
}.${BASE_DOMAIN}`;

type CreateCommsServiceArgs = {
...
export class CommsService extends pulumi.ComponentResource {
  ...
  constructor(
    name: string,
    {
      ...
      secretKeyArns,
      queueArns,
    }: CreateCommsServiceArgs,
    opts?: pulumi.ComponentResourceOptions
  ) {
    super('my:components:CommsService', name, {}, opts);
    this.tags = tags;
            ...
            service: {
              ...
              logConfiguration: {
                ...
                options: {
                  ...
                  dd_service: `comms-service-${stack}`,
                  dd_source: 'fargate',
                  dd_tags: `project:comms-service, env:${stack}`,
                  provider: 'ecs',
                },
              },
              portMappings: [
